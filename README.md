...
tetris.js 是一款基于原生 JavaScript 开发的纯前端俄罗斯方块游戏，无任何外部依赖，可直接在浏览器中运行。游戏完整实现了经典俄罗斯方块的核心功能，包含方块生成、移动、旋转、下落、碰撞检测、消行、升级、分数统计等能力，同时搭配丰富的界面渲染、动画特效与交互反馈。项目采用分层架构设计，模块化程度高，便于维护与功能扩展。
...
### 系统能力
- **操作回放**：游戏结束后，可完整回看整局游戏的操作过程
- **AI 操控**：具备多步预判能力，通过棋盘评估算法选择最优落子方案，并区分不同难度。
  演示视频：https://www.bilibili.com/video/BV1GPG86KEcy/?vd_source=8d9b68dd3ed316bb9b3a13e3f3f778eb
- **本地存储**：自动持久化保存游戏最高分
