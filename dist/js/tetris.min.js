var tetris = (() => {
  ...
  var ze = class {
      ...
      sequence(e) {
        let r = [],
          o = 0;
        for (let s of e) {
          let { fn: i, delay: d = 0 } = s;
          ((o += d), r.push(this.delay(i, o)));
        }
        return r;
      }
      cancel(e) {
        let r = this.tasks.find((o) => o.id === e);
        r && ((r.cancelled = !0), (this.dirty = !0));
      }
      clear() {
      ...
      _executeDueTasks(e) {
        for (; this.tasks.length > 0 && this.tasks[0].time <= e; ) {
          ...
          r.cancelled ||
            (r.type === 'delay'
              ? this._runDelayTask(r)
              : r.type === 'interval' && this._runIntervalTask(r, e));
        }
      }
      _runDelayTask(e) {
        e.fn(e);
      }
      _runIntervalTask(e, r) {
        let o = 0;
        for (; e.nextTime <= r && !e.cancelled && o < this.maxCatchUp; )
          (o++, e.fn(e), (e.nextTime += e.interval));
        (o >= this.maxCatchUp && (e.nextTime = r + e.interval),
  ...
  var Y0 = {
      ...
      off(t, e) {
        ...
        r && (r.delete(e), r.size === 0 && this.events.delete(t));
      },
      emit(t, e) {
        let r = this.events.get(t);
        if (r) for (let o of r) $(o) && o(e);
      },
      clear() {
        this.events.clear();
      },
  ...
  var Q0 = (t, e, r, o = {}) => {
      ...
      let { Context: s } = t,
        {
          ...
          gate: n = 1,
          articulation: c = {},
          startTime: a = s.currentTime,
        } = o,
        l = s.createOscillator(),
        u = s.createGain();
      ((l.type = d), l.frequency.setValueAtTime(e, a));
      let E = (r / 1e3) * n,
        {
          attackTime: m = 0.003,
          releaseTime: T = 0.02,
      ...
      (u.gain.setValueAtTime(1e-4, R),
        u.gain.linearRampToValueAtTime(i, w),
        u.gain.linearRampToValueAtTime(i * A, X),
        u.gain.exponentialRampToValueAtTime(1e-4, j),
        l.connect(u),
    ...
    L = Q0;
  var j0 = (t, e = !1) => (e ? 'perfect' : t === 4 ? 'tetris' : 'combo'),
    Fe = class extends v {
      constructor(e) {
        super(e);
      }
      ...
      FALL = () => L(this, 180, 200);
      CLEAR = (e = 1, r = 1, o = !1) => {
        let s = Math.min(Math.floor((r - 1) / 16), 15),
          i = Mt[s],
          d = Gt[s],
          n = [260, 300, 380],
          c = [0.32, 0.3, 0.25],
          ...
          m = i[p].filter((w) => w > 0).map((w) => w + u.shift * 12),
          T = [],
          { Context: A, Scheduler: R } = this;
        for (let [w, X] of m.entries())
          T.push({
            fn: () => {
              let j = A.currentTime;
              L(this, X, n[w] * u.speed * d.spdMul, {
                volume: c[w] * u.volume * d.volMul,
                wave: d.wave,
                startTime: j + a[w] / 1e3,
              });
            },
      ...
      LEVEL_UP = () => {
        let { Context: e, Scheduler: r } = this,
          o = e.currentTime;
        r.sequence([
          { fn: () => L(this, 523, 220) },
  ...
  var Z0 = [
    ...
    es = [0, 100, 300, 500, 800, 1200],
    ts = '"Press Start 2P", monospace, sans-serif',
    rs = 256,
    os = {
      CLEAR_LINE_SCORES: es,
      FONT_FAMILY: ts,
      AI_ALLOWED_ACTIONS: Z0,
      MAX_LEVEL: rs,
    },
    x = os;
  var ss = {
  ...
  var _s = {
    ...
    Kt = _s;
  var gs = [wt, Pt, It, bt, Bt, Nt, kt, Ht, Ut, zt, Vt, Ft, Wt, $t, Yt, Kt],
    We = gs;
  var Ss = 0.12,
    Ts = 25,
    Ls = (t, e, r = {}) => {
      ...
      let l = 0,
        ...
        E = () => {
          let T = a.currentTime + Ss;
          for (; u < T; ) {
            let A = e[l];
            p(A, u);
            let R = A.dur * o;
            ((u += R / 1e3), (l = (l + 1) % e.length));
          }
        };
      t.bgmSchedulerId = c.interval(E, Ts);
    },
    Xt = Ls;
  var As = (t, e) => {
      let { length: r } = We,
        { MAX_LEVEL: o } = x,
        s = Math.floor(o / r),
        i = Math.min(Math.floor((e - 1) / s), r - 1);
      return We[i];
    },
    Rs = (t, e = 1) => {
      let r = As(t, e),
      ...
      Xt(t, o, { duration: s, volume: i, wave: d, gate: n, articulation: c });
    },
    pe = Rs;
  var Cs = (t) => {
      (t.Scheduler.cancel(t.bgmSchedulerId), (t.bgmSchedulerId = 0));
    },
    Ee = Cs;
  var xs = (t, e) => {
      t.bgmSchedulerId === 0 ? pe(t, e) : Ee(t);
    },
    Jt = xs;
  var de = (t) => ({ CLEAR: `animations:${t}:clear` }),
    g = () => ({
    ...
    H = (t) => ({ START: `ai:${t}:start`, STOP: `ai:${t}:stop` }),
    Z = (t) => ({
      CLEAR: `command:queue:${t}:clear`,
      ENQUEUE: `command:queue:${t}:enqueue`,
    }),
    h = (t) => ({
      UPDATE_STATE: `game:${t}:update:state`,
      UPDATE_MODE: `game:${t}:update:mode`,
      UPDATE_LEVEL: `game:${t}:update:level`,
      UPDATE_GAMEPAD_CONNECTED: `game:${t}:update:gamepad:connected`,
      SWITCH_CONTROLLER: `game:${t}:swtich:controller`,
      UPDATE_HUD: `game:${t}:update:hud`,
      SAVE_HIGH_SCORE: `game:${t}:save:high:score`,
      SELECT_LEVEL: `game:${t}:select:level`,
      SWITCH_TO_DIFFICULTY: `game:${t}:switch:difficulty`,
      SELECT_DIFFICULTY: `game:${t}:select:difficulty`,
      SWITCH_TO_MAIN_MENU: `game:${t}:switch:to:main:menu`,
      BEGIN: `game:${t}:begin`,
      START: `game:${t}:start`,
      TOGGLE_PAUSED: `game:${t}:toggle:paused`,
      RESET: `game:${t}:reset`,
      RESTART: `game:${t}:restart`,
      OVER: `game:${t}:over`,
      GET_GHOST_POSITION: `game:${t}:get:ghost:position`,
      BLOCK_MOVE: `game:${t}:block:move`,
      BLOCK_ROTATE: `game:${t}:block:rotate`,
      BLOCK_DROP: `game:${t}:block:drop`,
      BLOCK_TICK: `game:${t}:block:tick`,
      BLOCK_SPAWN: `game:${t}:block:spawn`,
      BLOCK_HOLD: `game:${t}:block:hold`,
      START_COUNTDOWN: `game:${t}:start:countdown`,
      START_PAUSED: `game:${t}:start:paused`,
      STOP_PAUSED: `game:${t}:stop:paused`,
      START_CLEAR_LINES: `game:${t}:start:clear:lines`,
      START_CLEAR_SCORE: `game:${t}:start:clear:score`,
      START_LEVEL_UP: `game:${t}:start:level:up`,
      START_LANDING_FLASH: `game:${t}:start:landing:flash`,
      TOGGLE_BGM: `game:${t}:toggle:bgm`,
      REPLAY_PREPARE: `game:${t}:replay:prepare`,
    }),
    C = (t) => ({
      START_RECORD: `replay:${t}:start:record`,
      STOP_RECORD: `replay:${t}:stop:record`,
      ADD_RECORD: `replay:${t}:add:record`,
      ADD_PIECE: `replay:${t}:add:piece`,
      START_PLAY: `replay:${t}:start:play`,
      RESET: `replay:${t}:reset`,
      GAME_OVER: `replay:${t}:game:over`,
      STOP_CLEAR_LINES: `replay:${t}:stop:clear:lines`,
    }),
    _ = (t) => ({
      UPDATE_MODE: `ui:${t}:update:mode`,
      UPDATE_CONTROLLER: `ui:${t}:update:controller`,
      UPDATE_HUD: `ui:${t}:update:hud`,
      RESIZE: `ui:${t}:resize`,
      RENDER_NEXT_PIECE: `ui:${t}:render:next:piece`,
      RENDER_HOLD_PIECE: `ui:${t}:render:hold:piece`,
      CLEAR_NEXT_PIECE: `ui:${t}:clear:next:piece`,
      CLEAR_HOLD_PIECE: `ui:${t}:clear:hold:piece`,
      RENDER_GHOST_PIECE: `ui:${t}:render:ghost:piece`,
      RENDER_COUNTDOWN: `ui:${t}:render:countdown`,
      RENDER_CLEAR_LINES: `ui:${t}:render:clear:lines`,
      RENDER_CLEAR_SCORE: `ui:${t}:render:clear:score`,
      RENDER_LEVEL_UP: `ui:${t}:render:level:up`,
      RENDER_LANDING_FLASH: `ui:${t}:render:landing:flash`,
    });
  var $e = class extends v {
      ...
      initialize(e) {
        ...
        ((this.Context = r),
          (this.Sounds = new Dt({ ...e, Context: r })),
      ...
      _onPlaySound = ({ sound: e, lines: r, level: o }) => {
        let { Sounds: s } = this,
          i = s[e];
        $(i) && i(r, o);
      };
  ...
  var Ye = class extends v {
      ...
      subscribe() {
        let {
          ...
          n = h(s.id);
        (this.on(n.UPDATE_STATE, this._onUpdateState),
          this.on(n.UPDATE_MODE, this._onUpdateMode),
          this.on(n.UPDATE_LEVEL, this._onUpdateLevel),
          this.on(n.UPDATE_GAMEPAD_CONNECTED, this._onUpdateGamepadConnected),
          this.on(n.SWITCH_CONTROLLER, this._onSwitchController),
          this.on(n.UPDATE_HUD, this._onUpdateHud),
          this.on(n.SAVE_HIGH_SCORE, this._onSaveHighScore),
          this.on(n.SELECT_LEVEL, this._onSelectLevel),
          this.on(n.SWITCH_TO_DIFFICULTY, this._onSwitchToDifficulty),
          this.on(n.SELECT_DIFFICULTY, this._onSelectDifficulty),
          this.on(n.SWITCH_TO_MAIN_MENU, this._onSwitchToMainMenu),
          this.on(n.BEGIN, this._onGameBegin),
          this.on(n.START, this._onGameStart),
          this.on(n.TOGGLE_PAUSED, this._onTogglePaused),
          this.on(n.RESTART, this._onGameRestart),
          this.on(n.RESET, this._onGameReset),
          this.on(n.OVER, this._onGameOver),
          this.on(n.GET_GHOST_POSITION, this._onGetGhostPosition),
          this.on(n.BLOCK_MOVE, this._onBlockMove),
          this.on(n.BLOCK_ROTATE, this._onBlockRotate),
          this.on(n.BLOCK_DROP, this._onBlockDrop),
          this.on(n.BLOCK_TICK, this._onBlockTick),
          this.on(n.BLOCK_SPAWN, this._onBlockSpawn),
          this.on(n.BLOCK_HOLD, this._onBlockHold),
          this.on(n.START_COUNTDOWN, this._onStartCountdown),
          this.on(n.START_PAUSED, this._onStartPaused),
          this.on(n.STOP_PAUSED, this._onStopPaused),
          this.on(n.START_CLEAR_LINES, this._onStartClearLines),
          this.on(n.START_CLEAR_SCORE, this._onStartClearScore),
          this.on(n.START_LEVEL_UP, this._onStartLevelUp),
          this.on(n.START_LANDING_FLASH, this._onStartLandingFlash),
          this.on(n.TOGGLE_BGM, this._onToggleBGM),
          this.on(n.REPLAY_PREPARE, this._onReplayPrepare),
          r.subscribe(),
          e.subscribe(),
          o.subscribe(),
          i.subscribe(),
      ...
      unsubscribe() {
        let {
          ...
          n = h(s.id);
        (this.off(n.UPDATE_STATE, this._onUpdateState),
          this.off(n.UPDATE_MODE, this._onUpdateMode),
          this.off(n.UPDATE_LEVEL, this._onUpdateLevel),
          this.off(n.UPDATE_GAMEPAD_CONNECTED, this._onUpdateGamepadConnected),
          this.off(n.SWITCH_CONTROLLER, this._onSwitchController),
          this.off(n.UPDATE_HUD, this._onUpdateHud),
          this.off(n.SAVE_HIGH_SCORE, this._onSaveHighScore),
          this.off(n.SELECT_LEVEL, this._onSelectLevel),
          this.off(n.SWITCH_TO_DIFFICULTY, this._onSwitchToDifficulty),
          this.off(n.SELECT_DIFFICULTY, this._onSelectDifficulty),
          this.off(n.SWITCH_TO_MAIN_MENU, this._onSwitchToMainMenu),
          this.off(n.BEGIN, this._onGameBegin),
          this.off(n.START, this._onGameStart),
          this.off(n.TOGGLE_PAUSED, this._onTogglePaused),
          this.off(n.RESTART, this._onGameRestart),
          this.off(n.RESET, this._onGameReset),
          this.off(n.OVER, this._onGameOver),
          this.off(n.GET_GHOST_POSITION, this._onGetGhostPosition),
          this.off(n.BLOCK_MOVE, this._onBlockMove),
          this.off(n.BLOCK_ROTATE, this._onBlockRotate),
          this.off(n.BLOCK_DROP, this._onBlockDrop),
          this.off(n.BLOCK_TICK, this._onBlockTick),
          this.off(n.BLOCK_SPAWN, this._onBlockSpawn),
          this.off(n.BLOCK_SPAWN, this._onBlockHold),
          this.off(n.START_COUNTDOWN, this._onStartCountdown),
          this.off(n.START_PAUSED, this._onStartPaused),
          this.off(n.STOP_PAUSED, this._onStopPaused),
          this.off(n.START_CLEAR_LINES, this._onStartClearLines),
          this.off(n.START_CLEAR_SCORE, this._onStartClearScore),
          this.off(n.START_LEVEL_UP, this._onStartLevelUp),
          this.off(n.START_LANDING_FLASH, this._onStartLandingFlash),
          this.off(n.TOGGLE_BGM, this._onToggleBGM),
          this.off(n.REPLAY_PREPARE, this._onReplayPrepare),
          r.unsubscribe(),
          e.unsubscribe(),
          o.unsubscribe(),
          i.unsubscribe(),
      ...
      _onSwitchController = () => {
        ...
        (e.setController(s),
          ...
          this.emit(d.UPDATE_CONTROLLER, { controller: s }));
      };
      _onUpdateState = (e) => {
        let { Store: r } = this,
          { stateHandler: o } = e;
      ...
      _onUpdateMode = (e) => {
        let { Store: r, Game: o } = this,
          { mode: s } = e,
          i = _(o.id);
        (this.emit(i.UPDATE_MODE, { mode: s }), r.setMode(s));
      };
      _onUpdateLevel = (e) => {
        let { Store: r } = this,
          { level: o } = e;
      ...
      _onSaveHighScore = () => {
        let { Store: e, Game: r } = this;
        r.saveHighScore(e.getScore());
      };
      _onSelectLevel = (e) => {
        let { Store: r, Game: o } = this,
          { level: s } = e,
          i = _(o.id);
        (o.selectLevel(s), this.emit(i.UPDATE_HUD, { state: r.getState() }));
      };
      _onSwitchToDifficulty = () => {
        let { Game: e } = this,
          r = _(e.id);
        (this.emit(r.UPDATE_MODE, { mode: 'difficulty' }),
          e.switchToDifficulty());
      };
      _onSelectDifficulty = (e) => {
        let { Game: r } = this,
          { difficulty: o } = e;
        r.selectDifficulty(o);
      };
      _onSwitchToMainMenu = () => {
        let { Game: e } = this;
        e.switchToMainMenu();
      ...
      _onGameStart = () => {
        let { Store: e, Game: r } = this,
          o = H(r.id);
        (r.start(), e.getController() === 'ai' && this.emit(o.START));
      };
      _onTogglePaused = () => {
        let { Store: e, Game: r } = this,
          o = H(r.id);
        if ((r.togglePause(), e.getController() === 'ai')) {
      ...
      _onToggleBGM = () => {
        let { Store: e } = this,
          r = g();
        this.emit(r.TOGGLE_BGM, { level: e.getLevel() });
      };
      _onReplayPrepare = () => {
        let { Store: e, Game: r } = this,
          o = r.id,
        ...
        (e.resetBoard(),
          ...
          this.emit(d.BLOCK_SPAWN));
      };
      _onUpdateGamepadConnected = (e) => {
        let { Store: r } = this,
          { connected: o } = e;
        r.setGamepadConnected(o);
      };
      _onStartCountdown = () => {
        let { Game: e } = this;
        e.startCountdown();
  ...
  var Os = {
    ...
    Zt = Os;
  var ys = '#00c8ff',
    Ms = 'rgba(0, 200, 255, 0.3)',
    Gs = '#f1fa04',
    Ds = 'rgba(255, 255, 0, 0.3)',
    ws = '#d31ac1',
    Ps = 'rgba(211, 26, 193, 0.3)',
    Is = '#5050ff',
    bs = 'rgba(80, 80, 255, 0.3)',
    Bs = '#ffa500',
    Ns = 'rgba(255, 127, 0, 0.3)',
    ks = '#0afa04',
    Hs = '#5c9d31',
    Us = 'rgba(0, 255, 0, 0.3)',
    zs = '#ff3b30',
    Vs = 'rgba(255, 59, 48, 0.3)',
    Fs = '#e64a19',
    Ws = 'rgba(230, 74, 25, 0.3)',
    $s = '#444',
    Ys = 'rgba(0, 0, 0, 0.3)',
    Ks = '#fff',
    Xs = 'rgba(255, 255, 255, 0.3)',
    Js = '#ff4fa3',
    Qs = 'rgba(255, 79, 163, 0.3)',
    js = '#7b34eb',
    Zs = 'rgba(123, 52, 235, 0.3)',
    e1 = '#0cc0df',
    t1 = 'rgba(12, 192, 223, 0.3)',
    r1 = '#ff6b6b',
    o1 = '#ffa502',
    s1 = '#ffd700',
    i1 = '#ff7f50',
  ...
  var {
    ...
    ui = [ii, ni, di, ci, ai, li, fi],
    hi = (t, e, r) => {
      let o = r || ui,
        s = [];
      for (let i = 0; i < t; i += 1) {
        let d = Array.from({ length: e }).fill('');
        for (let l = 0; l < e; l += 1)
          d[l] = o[Math.floor(Math.random() * o.length)];
        let n = e - 3,
          c = 1 + Math.floor(Math.random() * n),
          a = new Set();
        for (; a.size < c; ) a.add(Math.floor(Math.random() * e));
        for (let l of a) d[l] = '';
        s.push(d);
      }
      return s;
  ...
  var mi = (t, e, r) => {
      ...
      let s = er(e, r),
        i = o - e;
      for (let d = 0; d < s.length; d++) i + d >= 0 && (t[i + d] = [...s[d]]);
    },
    tr = mi;
  var Ke = class {
      constructor(e) {
      ...
      initialize(e) {
        let { GameState: r, cols: o, rows: s } = e;
        ((this.defaults = structuredClone(r)),
      ...
      setState(e) {
        this.state = { ...this.state, ...($(e) ? e(this.state) : e) };
      }
      resetState() {
      ...
      resetBoard() {
        let { cols: e, rows: r } = this.options;
        this.state.board = Array.from({ length: r }, () =>
          Array.from({ length: e }).fill(0),
        );
      }
      generateBoard() {
        let e = { easy: 0, normal: 3, hard: 6, expert: 9 },
          { board: r, difficulty: o } = this.state,
          s = r[0].length,
          i = e[o] || 0;
        return (tr(r, i, s), r);
      }
      ...
      getHub() {
        ...
        return { source: e, lines: r, level: o, combo: s, comboScore: i };
      }
      setHud(e) {
        let { score: r, lines: o, level: s, combo: i, comboScore: d } = e;
        ((this.state.score = r),
          (this.state.lines = o),
          (this.state.level = s),
          (this.state.combo = i),
          (this.state.comboScore = d));
      }
      getScore() {
  ...
  var Je = class extends v {
      ...
      render() {
        this.#r &&
          ((this.#o = this.#e.slice().toSorted((e, r) => e.layer - r.layer)),
          (this.#r = !1));
      ...
      hasBlocking(e = []) {
        let r = e.length > 0;
        for (let o of this.#e)
          if (!(o._finished || !o.blocking) && (!r || e.includes(o.name)))
            return !0;
        return !1;
      }
      clear() {
      ...
      subscribe() {
        let { Game: e } = this,
          r = de(e.id);
        this.on(r.CLEAR, this._onClear);
      }
      unsubscribe() {
        let { Game: e } = this,
          r = de(e.id);
        this.off(r.CLEAR, this._onClear);
      }
      _onClear = () => {
        this.clear();
      };
  ...
  var Qe = class extends v {
      ...
      initialize(e) {
        let { board: r, next: o, hold: s } = e;
        ((this.gameBoard = document.querySelector(`#${r}`)),
          (this.gameBoardContext = this.gameBoard.getContext('2d')),
          (this.nextPiece = document.querySelector(`#${o}`)),
          (this.nextPieceContext = this.nextPiece.getContext('2d')),
          (this.holdPiece = document.querySelector(`#${s}`)),
  ...
  var Ei = (t) => {
      ...
      return {
        controller: document.querySelector(`#${e}`),
        score: document.querySelector(`#${r}`),
        lines: document.querySelector(`#${o}`),
        level: document.querySelector(`#${s}`),
        combo: document.querySelector(`#${i}`),
        highScore: document.querySelector(`#${d}`),
      };
    },
    nr = Ei;
  var z = (t, e, r = 0) => (t.textContent = r ? Y(e, r) : String(e)),
    dr = (t, e, r) => {
      if (t.visual === t.target) return;
      let o = t.target - t.visual,
        s = Math.ceil(Math.abs(o) * 0.1);
      (o > 0
        ? ((t.visual += s), t.visual > t.target && (t.visual = t.target))
        : ((t.visual -= s), t.visual < t.target && (t.visual = t.target)),
        z(e, t.visual, r));
    },
    je = class {
      ...
      update(e) {
        ...
        ((o.target = Number(e.score) || 0),
          ...
          e.lines !== i.lines && (z(r.lines, e.lines, 2), (i.lines = e.lines)),
          e.level !== i.level && (z(r.level, e.level, 2), (i.level = e.level)),
          e.combo !== i.combo && (z(r.combo, e.combo, 2), (i.combo = e.combo)));
      }
      updateController(e) {
        z(this.elements.controller, e.toUpperCase());
      }
      tick() {
        let { elements: e, scoreTracker: r, highScoreTracker: o } = this;
        (dr(r, e.score, 5), dr(o, e.highScore, 5));
      }
      reset() {
        let {
          elements: e,
    ...
    cr = je;
  function qi(t) {
    let { gameBoard: e, gameBoardContext: r } = t,
      { width: o, height: s } = e;
    r.clearRect(0, 0, o, s);
  }
  var y = qi;
  var vi = (t, e) => {
      let {
          ...
          size: n = 1,
          center: c = !0,
          baseline: a = 'alphabetic',
          stroke: l = !1,
          lineWidth: u = 2,
          alpha: p = 1,
        } = e,
        { FONT_FAMILY: E } = x,
        { gameBoardContext: m, fontSize: T } = t;
      (m.save(),
        p < 1 && (m.globalAlpha = p),
        c && (m.textAlign = 'center'),
        (m.textBaseline = a),
        (m.font = `${T * n}px ${E}`),
        l &&
          ((m.strokeStyle = d || i), (m.lineWidth = u), m.strokeText(r, o, s)),
        (m.fillStyle = i),
        m.fillText(r, o, s),
        p < 1 && (m.globalAlpha = 1),
        m.restore());
  ...
  var _i = (t) => {
      let { GREEN: e } = f,
        { gameBoard: r } = t,
        { width: o, height: s } = r;
      S(t, { text: 'TETRIS.JS', x: o / 2, y: s * 0.1, color: e, size: 1.1 });
    },
    M = _i;
  var gi = (t) => {
      let { GREEN: e } = f,
  ...
  var Si = (t, e, r) => {
      let { GREEN: o } = f,
        { gameBoard: s } = t,
        { width: i } = s;
      S(t, { text: String(e), x: i / 2, y: r, color: o, size: 3, center: !0 });
    },
    qe = Si;
  var Ti = (t) => {
      let { WHITE: e } = f,
  ...
  var Ai = (t, e) => {
      let { RGBA_BLACK: r } = f,
        { gameBoard: o, gameBoardContext: s } = t,
        { width: i, height: d } = o;
      (s.save(), (s.fillStyle = e || r), s.fillRect(0, 0, i, d), s.restore());
    },
    G = Ai;
  var { RGBA_WHITE: ve } = f,
    Ri = {
    ...
    K = Ri;
  var Ze = new Map(),
    Ci = (t) => `data:image/svg+xml;charset=utf-8,${encodeURIComponent(t)}`,
    P = (t) => {
      let e = Ze.get(t);
      if (e) return e;
      let r = new Image();
      return ((r.src = Ci(t)), Ze.set(t, r), r);
    },
    xi = () => {
      Ze.clear();
    },
  ...
  var Oi = (t, e) => {
      let { gameBoardContext: r } = t,
        { img: o, x: s, y: i, size: d } = e;
      o.complete && (r.save(), r.drawImage(o, s, i, d, d), r.restore());
    },
    J = Oi;
  var yi = (t, e) => {
      let { gameBoard: r } = t,
      ...
      switch (e) {
        case 'main-menu':
        case 'countdown': {
          ((n = P(K.tetris)), (c = o), (a = o / 2 - c / 2), (l = s - c));
          break;
        }
        case 'playing': {
          (i >= 0 && i <= 8
  ...
  var Pi = (t, e) => {
      let { WHITE: r } = f,
        ...
        { width: s, height: i } = o,
        d = 'E/N/H/X KEY';
      (e.gamepadConnected && (d = 'A/B/Y/X KEY'),
        S(t, {
          text: d,
          x: s / 2,
          y: i * 0.58,
  ...
  var {
    ...
    sn = {
      Teal: { stroke: _e, face: Fi, secondHand: ge },
      Violet: { stroke: en, face: tn, secondHand: _e },
      Yellow: { stroke: tt, face: ji, secondHand: ge },
      Pink: { stroke: Wi, face: $i, secondHand: tt },
      Purple: { stroke: zi, face: Vi, secondHand: _e },
      Green: { stroke: ge, face: Xi, secondHand: _e },
      Blue: { stroke: Ji, face: Qi, secondHand: tt },
      Coral: { stroke: ki, face: Hi, secondHand: et },
      Orange: { stroke: Yi, face: Ki, secondHand: rt },
      Cyan: { stroke: rn, face: on, secondHand: ge },
      White: { stroke: et, face: Ui, secondHand: rt },
      Red: { stroke: rt, face: Zi, secondHand: et },
    },
    Se = sn;
  var nn = (t, e = 'yyyy-MM-dd HH:mm:ss') => {
      let r = t.getFullYear(),
        o = t.getMonth() + 1,
        s = t.getDate(),
        i = t.getHours(),
        ...
        p = e;
      for (let E of Object.keys(u)) p = p.replaceAll(new RegExp(E, 'g'), u[E]);
      return p;
    },
    Sr = nn;
  var dn = (t) =>
  ...
  var mn = (t, e, r) => {
      ...
      for (let d = 0; d < 12; d++)
        (o.save(),
          o.rotate((d * Math.PI) / 6),
          o.beginPath(),
          o.arc(0, -i, s, 0, Math.PI * 2),
          (o.fillStyle = r.stroke),
          o.fill(),
          o.restore());
    },
    Or = mn;
  var pn = (t, e, r, o) => {
      let { gameBoardContext: s } = t,
        { hAng: i, mAng: d, sAng: n } = r;
      (s.save(),
        s.rotate(i),
        (s.lineWidth = 5),
  ...
  var En = (t, e, r) => {
    ...
    Mr = En;
  var qn = (t, e) => {
      let { gameBoard: r, gameBoardContext: o } = t,
        { width: s, height: i } = r,
        d = s / 2,
        n = i / 2.2,
        c = Math.floor(s * 0.3),
    ...
    Gr = qn;
  var vn = (t, e, r, o) => {
      let { gameBoardContext: s, blockSize: i } = t,
        { BLACK: d } = f,
        n = 1,
        c = i - n,
        a = e * i + n,
      ...
      ((s.fillStyle = o),
        s.fillRect(a, l, c, c),
        (s.strokeStyle = d),
        s.strokeRect(a, l, c, c));
    },
    Dr = vn;
  var _n = (t, e) => {
      let r = Number.parseInt(t.slice(1, 3), 16),
        o = Number.parseInt(t.slice(3, 5), 16),
        s = Number.parseInt(t.slice(5, 7), 16),
        i = Math.floor(r * (1 - e)),
        d = Math.floor(o * (1 - e)),
        n = Math.floor(s * (1 - e));
      return `#${i.toString(16).padStart(2, '0')}${d.toString(16).padStart(2, '0')}${n.toString(16).padStart(2, '0')}`;
    },
    O = _n;
  var gn = (t, e) => {
      let r = t;
      t.length === 4 && (r = `#${t[1]}${t[1]}${t[2]}${t[2]}${t[3]}${t[3]}`);
      let o = Number.parseInt(r.slice(1, 3), 16),
        s = Number.parseInt(r.slice(3, 5), 16),
        i = Number.parseInt(r.slice(5, 7), 16);
      return `rgba(${o}, ${s}, ${i}, ${e})`;
    },
    V = gn;
  var Sn = (t, e, r, o) => {
      ...
      ((s.fillStyle = V(o, 0.65)),
        ...
        (s.lineWidth = 1),
        s.strokeRect(c + 0.5, a + 0.5, n - 1, n - 1),
        s.save(),
        s.beginPath(),
        s.rect(c, a, n, n),
        s.clip());
      let l = s.createLinearGradient(c, a, c, a + n);
      (l.addColorStop(0, V('#FFFFFF', 0.15)),
        l.addColorStop(0.25, V('#FFFFFF', 0)),
        (s.fillStyle = l),
        s.fillRect(c, a, n, n));
      let u = (e * 31 + r * 17 + n * 13) % 1e3;
      for (let p = 0; p < n * n * 0.3; p++) {
        let E = c + ((u + p * 7) % n),
          m = a + ((u + p * 11) % n),
          T = 0.03 + ((u + p * 3) % 8) * 0.01;
        ((s.fillStyle = V('#FFFFFF', T)), s.fillRect(E, m, 1, 1));
      }
      (s.restore(),
        (s.strokeStyle = O(o, 0.45)),
        (s.lineWidth = 2),
        s.strokeRect(c + 0.5, a + 0.5, n - 1, n - 1));
    },
    wr = Sn;
  var Tn = (t, e, r, o) => {
      ...
      ((s.fillStyle = V(o, 0.65)),
        ...
        s.rect(a, l, c, c),
        s.clip());
      let u = s.createLinearGradient(a, l, a, l + c);
      (u.addColorStop(0, V(d, 0.25)),
        u.addColorStop(0.4, V(d, 0)),
        (s.fillStyle = u),
        s.fillRect(a, l, c, c),
        s.restore(),
        (s.strokeStyle = O(o, 0.35)),
        (s.lineWidth = 2),
        s.strokeRect(a + 0.5, l + 0.5, c - 1, c - 1));
    },
    Pr = Tn;
  var Ln = (t, e) => {
      let r = Number.parseInt(t.slice(1, 3), 16),
        o = Number.parseInt(t.slice(3, 5), 16),
        s = Number.parseInt(t.slice(5, 7), 16),
        i = Math.min(255, Math.floor(r + (255 - r) * e)),
        d = Math.min(255, Math.floor(o + (255 - o) * e)),
  ...
  var An = (t, e, r, o) => {
      let { gameBoardContext: s, blockSize: i } = t,
        ...
        c = e * i + d,
        a = r * i + d;
      ((s.fillStyle = O(o, 0.15)),
        s.fillRect(c, a, n, n),
        (s.strokeStyle = f.BLACK),
        (s.lineWidth = 1),
        s.strokeRect(c, a, n, n));
      let l = F(o, 0.25),
        u = Math.max(2, Math.floor(n / 4));
      ((s.fillStyle = l), s.fillRect(c, a, n, u), s.fillRect(c, a, u, n));
    },
    Ir = An;
  var Rn = (t, e, r, o) => {
      let { RGBA_BLACK: s, RGBA_WHITE: i } = f,
        { gameBoardContext: d, blockSize: n } = t,
        c = e * n,
        a = r * n,
        l = n,
        u = n,
        p = F(o, 0.15),
        E = O(o, 0.2),
        m = d.createLinearGradient(c, a, c, a + u);
      (m.addColorStop(0, p),
        ...
        d.beginPath(),
        d.moveTo(c, a),
        d.lineTo(c + l * 0.3, a + u * 0.5),
        d.lineTo(c, a + u),
        d.fill(),
        (d.fillStyle = s),
        d.beginPath(),
        d.moveTo(c + l, a),
        d.lineTo(c + l * 0.7, a + u * 0.5),
        d.lineTo(c + l, a + u),
        d.fill(),
        (d.strokeStyle = s),
        (d.lineWidth = 1),
        d.strokeRect(c + 0.5, a + 0.5, l - 1, u - 1));
    },
    br = Rn;
  var Cn = (t, e, r, o) => {
      let { gameBoardContext: s, blockSize: i } = t,
    ...
    Br = Cn;
  var Q = (t) => {
      let { ctx: e, px: r, py: o, w: s, h: i, u: d, steps: n, color: c } = t;
      ((e.fillStyle = c),
        e.fillRect(r + d * n, o + d * n, s - d * n * 2, i - d * n * 2));
    },
    Nr = (t) => {
      let {
        ctx: e,
      ...
      (Q({ ctx: e, px: r, py: o, w: s, h: i, u: d, steps: 0, color: c }),
        Q({ ctx: e, px: r, py: o, w: s, h: i, u: d, steps: 1, color: n }),
        Q({ ctx: e, px: r, py: o, w: s, h: i, u: d, steps: 2, color: c }));
    },
    xn = (t, e, r, o, s = 'square') => {
      let { gameBoardContext: i, blockSize: d } = t,
        n = d / 8,
        c = e * d,
        a = r * d,
        l = d,
      ...
      switch (s) {
        case 'jay': {
          (Nr({
            ...
            Q({ ctx: i, px: c, py: a, w: l, h: u, u: n, steps: 3, color: E }));
          break;
        }
        case 'ell': {
          (Q({ ctx: i, px: c, py: a, w: l, h: u, u: n, steps: 0, color: p }),
            Q({ ctx: i, px: c, py: a, w: l, h: u, u: n, steps: 1, color: o }));
          break;
        }
        case 'tee': {
          (Q({ ctx: i, px: c, py: a, w: l, h: u, u: n, steps: 0, color: p }),
            Q({ ctx: i, px: c, py: a, w: l, h: u, u: n, steps: 1, color: o }),
            (i.fillStyle = E),
            i.fillRect(c + n * 2, a + n * 2, n, u - n * 4),
            i.fillRect(c + n * 2, a + n * 2, l - n * 4, n),
            (i.fillStyle = p),
            i.fillRect(c + n * 2, a + n * 5, l - n * 4, n),
            i.fillRect(c + n * 5, a + n * 3, n, u - n * 6));
          break;
        }
        default:
          Nr({
  ...
  var On = (t, e, r, o) => {
      let { gameBoardContext: s, blockSize: i } = t,
        ...
        E = O(o, 0.22);
      ((s.fillStyle = E),
        s.fillRect(d, n + a / 2, c, a / 2),
        (s.fillStyle = u),
        s.fillRect(d, n, c, a / 2),
        (s.fillStyle = l),
        s.beginPath(),
        s.moveTo(d, n),
        s.lineTo(d + c / 2, n + a / 2),
        s.lineTo(d, n + a),
        s.fill(),
        (s.fillStyle = p),
        s.beginPath(),
    ...
    Hr = On;
  var yn = (t, e, r, o) => {
      let { style: s = 'classic', pattern: i = 'square' } = t;
      switch (s) {
        case 'frosted': {
          wr(t, e, r, o);
          break;
  ...
  var Dn = {
      night_0_3: (t, e) => ({
        size: Math.floor(t * 0.48),
        x: t - Math.floor(t * 0.48) * 0.7,
        y: e / 2 - Math.floor(t * 0.48) * 1.4,
      }),
      morning_4_7: (t, e) => {
        let r = Math.floor(t * 0.52);
        return { size: r, x: t - r * 1.1, y: e / 2 - r * 1.7 };
      },
      morning_8_11: (t, e) => {
        let r = Math.floor(t * 0.58);
        return { size: r, x: t - r * 1.2, y: e / 2 - r * 1.75 };
      },
      noon_12_14: (t) => {
        let e = Math.floor(t * 0.68);
        return { size: e, x: t / 2 - e / 2, y: -e * 0.1 };
      },
      afternoon_14_16: (t, e) => {
        let r = Math.floor(t * 0.58);
        return { size: r, x: r * 0.2, y: e / 2 - r * 1.75 };
      },
      evening_17_19: (t, e) => {
        let r = Math.floor(t * 0.52);
        return { size: r, x: r * 0.1, y: e / 2 - r * 1.7 };
      },
      night_20_23: (t, e) => {
        let r = Math.floor(t * 0.48);
        return { size: r, x: -r * 0.3, y: e / 2 - r * 1.4 };
      },
    },
    wn = (t) =>
      t <= 3
    ...
    Pn = (t) => {
      let { gameBoard: e } = t,
        ...
        n = wn(s),
        c = Dn[n],
        { size: a, x: l, y: u } = c(r, o);
      J(t, { img: d, x: l, y: u, size: a });
    },
    Vr = Pn;
  var In = (t, e) => {
      let { rows: r, cols: o } = t;
      (y(t), Vr(t), D(t, 'playing'));
      for (let s = 0; s < r; s++)
        for (let i = 0; i < o; i++) e[s][i] && b(t, i, s, e[s][i]);
    },
    Fr = In;
  var bn = (t, e, r, o) => {
      let { shape: s, color: i } = e,
        { length: d } = s;
      for (let n = 0; n < d; n++)
        for (let c = 0; c < s[n].length; c++) s[n][c] && b(t, r + c, o + n, i);
      return !0;
    },
    Wr = bn;
  var Bn = (t, e) => {
      let { board: r, curr: o, cx: s, cy: i, level: d } = e;
      if ((r && Fr(t, r), o && d <= 9)) {
        let n = h(t.uuid);
        t.emit(n.GET_GHOST_POSITION, { curr: o, cy: i });
      }
      o && Wr(t, o, s, i);
    },
    re = Bn;
  var Nn = (t, e) => {
      (y(t), re(t, e), G(t), D(t, 'paused'), M(t), Tr(t), Gr(t), gr(t));
    },
    $r = Nn;
  var kn = (t, e) => {
      $r(t, e);
  ...
  var Un = (t) => {
    ...
    Ae = Un;
  var zn = (t, e) => {
      (y(t), re(t, e), G(t), D(t, 'game-over'), M(t), Le(t), Ae(t), te(t));
    },
    Kr = zn;
  var Vn = (t, e) => {
      Kr(t, e);
    ...
    Xr = Vn;
  var Fn = (t) => {
      let { nextPiece: e, nextPieceContext: r } = t,
        { width: o, height: s } = e;
      r.clearRect(0, 0, o, s);
    },
    Re = Fn;
  var Wn = (t, e) => {
      ...
      let { shape: a } = r,
        l = Math.ceil(n / 6),
        u = Math.floor((n - a[0].length * l) / 2),
        p = Math.floor((c - a.length * l) / 2);
      (Re(t), d.save(), d.translate(u, p));
      for (let E = 0; E < a.length; E++)
        for (let m = 0; m < a[E].length; m++)
          a[E][m] &&
            b(
              {
                gameBoardContext: d,
    ...
    Ce = Wn;
  var $n = (t) => {
      let { nextPiece: e, holdPieceContext: r } = t,
        { width: o, height: s } = e;
      r.clearRect(0, 0, o, s);
    },
    xe = $n;
  var Yn = (t, e) => {
      ...
      let { shape: a } = r,
        ...
        u = Math.floor((n - a[0].length * l) / 2),
        p = Math.floor((c - a.length * l) / 2);
      (xe(t), o.save(), o.translate(u, p));
      for (let E = 0; E < a.length; E++)
        for (let m = 0; m < a[E].length; m++)
          a[E][m] &&
            b(
              { gameBoardContext: o, blockSize: l, style: i, pattern: d },
              m,
              E,
              r.color,
            );
  ...
  var Xn = (t, e) => {
    ...
    Jr = Xn;
  var Jn = (t, e) => {
      (y(t), ye(t, e), G(t), D(t, 'game-over'), M(t), Le(t), Ae(t), te(t));
    },
    Qr = Jn;
  var Qn = (t, e) => {
      Qr(t, e);
  ...
  var t2 = (t, e) => {
      ...
      for (let c = 0; c < d.length; c++)
        for (let a = 0; a < d[c].length; a++) d[c][a] && b(t, s + a, i + c, n);
      r.globalAlpha = 1;
    },
    to = t2;
  var r2 = (t) => {
      let { gameBoard: e, nextPiece: r, holdPiece: o, rows: s, cols: i } = t,
        { innerWidth: d, innerHeight: n } = globalThis,
        c,
        a,
        l;
      if (d >= 480) {
        let p = n * 0.9;
        ((c = Math.floor(p / s)), (a = l = Math.min(d * 0.1, n * 0.18)));
      } else {
        let p = d * 0.64;
        ((c = Math.min(Math.floor(p / i), Math.floor((n * 0.68) / s))),
          (a = l = c * 5));
      }
      let u = Math.floor(c * s * 0.032);
      ((t.blockSize = c),
        (e.width = c * i),
        (e.height = c * s),
        (t.fontSize = u),
    ...
    ro = r2;
  var o2 = (t, e) => {
      let { gameBoardContext: r, cols: o } = t;
      for (let s of e.lines) {
        (r.save(), (r.globalAlpha = s.alpha));
        for (let i = 0; i < o; i++) b(t, i, s.y, s.color);
        r.restore();
      }
    },
    oo = o2;
  var s2 = (t, e) => {
      let { WHITE: r, YELLOW: o } = f,
        { gameBoard: s, blockSize: i } = t,
        { width: d } = s,
        { score: n, y: c, alpha: a, offsetY: l, combo: u, comboScore: p } = e;
      if (a <= 0) return;
      let E = d / 2,
        m = c * i + i / 2 - l;
      (u > 1 &&
  ...
  var i2 = (t, e, r = 1) => {
      let { YELLOW: o, BLACK: s } = f,
        { FONT_FAMILY: i } = x,
        { gameBoard: d, gameBoardContext: n, fontSize: c } = t,
        { width: a, height: l } = d;
      (n.save(),
        (n.textAlign = 'center'),
        ...
        (n.lineWidth = 6));
      let u = String(e);
      (n.strokeText(u, 0, 0), n.fillText(u, 0, 0), n.restore());
    },
    io = i2;
  var n2 = (t) => {
      let { GREEN: e, BLACK: r } = f,
        { gameBoard: o } = t,
        { width: s, height: i } = o;
      S(t, {
        text: 'GET READY!',
  ...
  var d2 = (t) => {
      let { gameBoard: e } = t,
        ...
        d = r / 2 - i / 2,
        n = o / 2 - i * 1.2;
      J(t, { img: s, x: d, y: n, size: i });
    },
    co = d2;
  var c2 = (t, e) => {
      let { number: r, scale: o } = e;
      (y(t), G(t), M(t), D(t, 'countdown'), co(t), no(t), io(t, r, o));
    },
    ao = c2;
  var a2 = (t, e) => {
      let { gameBoardContext: r } = t;
      for (let o of e)
        ((r.globalAlpha = o.alpha),
          (r.fillStyle = o.color),
          r.beginPath(),
          r.arc(o.x, o.y, o.radius, 0, Math.PI * 2),
          r.fill(),
          (o.x += o.vx),
          (o.y += o.vy),
          (o.alpha -= 0.024));
  ...
  function u2(t, e, r) {
    let { gameBoard: o } = t,
      { height: s } = o;
    (G(t), M(t), fo(t), qe(t, e, s / 1.85), uo(t), lo(t, r));
  }
  var ho = u2;
  var h2 = (t, e) => {
      if (!e) return;
      let { WHITE: r } = f,
        { gameBoardContext: o, blockSize: s } = t,
        { cells: i } = e;
      ((o.globalAlpha = 0.6), (o.fillStyle = r));
      for (let { x: d, y: n } of i) o.fillRect(d * s, n * s, s, s);
      o.globalAlpha = 1;
    },
    mo = h2;
  var ot = class extends v {
      ...
      initialize() {
        let { Game: e, Elements: r, Block: o } = this,
          { Hud: s, Canvas: i } = r;
        ((this.Hud = new cr(s)),
          (this.Canvas = new ir({ uuid: e.id, ...i, ...o })));
      }
      updateMode(e) {
  ...
  var st = class extends v {
      ...
      subscribe() {
        let { Game: e } = this,
          r = _(e.id);
        (this.on(r.UPDATE_MODE, this._onUpdateMode),
          this.on(r.UPDATE_HUD, this._onUpdateHud),
          this.on(r.UPDATE_CONTROLLER, this._onUpdateController),
          this.on(r.RESIZE, this._onResize),
          this.on(r.RENDER_NEXT_PIECE, this._onRenderNextPiece),
          this.on(r.RENDER_HOLD_PIECE, this._onRenderHoldPiece),
          this.on(r.CLEAR_NEXT_PIECE, this._onClearNextPiece),
          this.on(r.CLEAR_HOLD_PIECE, this._onClearHoldPiece),
          this.on(r.RENDER_GHOST_PIECE, this._onRenderGhostPiece),
          this.on(r.RENDER_COUNTDOWN, this._onRenderCountdown),
          this.on(r.RENDER_CLEAR_LINES, this._onRenderClearLines),
          this.on(r.RENDER_CLEAR_SCORE, this._onRenderClearScore),
          this.on(r.RENDER_LEVEL_UP, this._onRenderLevelUp),
          this.on(r.RENDER_LANDING_FLASH, this._onRenderLandingFlash));
      }
      unsubscribe() {
        let { Game: e } = this,
          r = _(e.id);
        (this.off(r.UPDATE_MODE, this._onUpdateMode),
          this.off(r.UPDATE_HUD, this._onUpdateHud),
          this.off(r.UPDATE_CONTROLLER, this._onUpdateController),
          this.off(r.RESIZE, this._onResize),
          this.off(r.RENDER_NEXT_PIECE, this._onRenderNextPiece),
          this.off(r.RENDER_HOLD_PIECE, this._onRenderHoldPiece),
          this.off(r.CLEAR_NEXT_PIECE, this._onClearNextPiece),
          this.off(r.CLEAR_HOLD_PIECE, this._onClearHoldPiece),
          this.off(r.RENDER_GHOST_PIECE, this._onRenderGhostPiece),
          this.off(r.RENDER_COUNTDOWN, this._onRenderCountdown),
          this.off(r.RENDER_CLEAR_LINES, this._onRenderClearLines),
          this.off(r.RENDER_CLEAR_SCORE, this._onRenderClearScore),
          this.off(r.RENDER_LEVEL_UP, this._onRenderLevelUp),
          this.off(r.RENDER_LANDING_FLASH, this._onRenderLandingFlash));
      }
      ...
      _onRenderClearLines = (e) => {
        let { state: r } = e,
          { UI: o } = this;
        o.renderClearLines(r);
      };
      _onRenderClearScore = (e) => {
        let { state: r } = e,
          { UI: o } = this;
        o.renderClearScore(r);
      };
      _onRenderLevelUp = (e) => {
        let { level: r, fireworks: o } = e,
          { UI: s } = this;
        s.renderLevelUp(r, o);
      };
  ...
  var it = class extends v {
      ...
      initialize(e) {
        let { Game: r } = this;
        ((this.Renderer = new po(e)),
          (this.Router = new Eo({ UI: this, Game: r })));
      }
      updateMode(e) {
      ...
      renderLevelUp(e, r) {
        this.Renderer.renderLevelUp(e, r);
      }
      renderLandingFlash(e) {
  ...
  var vo = { DAS: 10, ARR: 2 },
    ...
    nt = class extends v {
      ...
      initialize() {
        this.dasState = { dasTimer: -1, arrTimer: 0, direction: 0, active: !1 };
      }
      update() {
        let { dasState: e, Game: r } = this;
        if (
          !(!e.active || e.direction === 0) &&
          r.Store.getMode() === 'playing'
        ) {
          if (e.dasTimer < vo.DAS) {
            e.dasTimer++;
            return;
          }
          e.arrTimer >= vo.ARR
      ...
      _isBlocked(e) {
        ...
        return (
          !o ||
          (s === 'replay' && e !== 'enter') ||
          (i === 'ai' && s === 'playing' && !x.AI_ALLOWED_ACTIONS.includes(o))
        );
      }
      _onResize = () => {
        let { Game: e } = this,
          r = _(e.id);
        return (this.emit(r.RESIZE), this);
      };
      _onKeydown = (e) => {
        let { Game: r } = this,
          o = e.key?.toLowerCase();
      ...
      _onKeyup = (e) => {
        ...
        return (
          ((r === 'arrowleft' && this.dasState.direction === -1) ||
            (r === 'arrowright' && this.dasState.direction === 1)) &&
  ...
  var U = {
    ...
    ct = class extends v {
      ...
      update(e) {
        return (
          ...
          this.activeGamepad ? (this._collectCommands(e), this) : this
        );
      }
      addEventListeners() {
      ...
      _onConnect = (e) => {
        ...
        ((this.activeGamepadIndex = r.index),
          (this.curBtnMap = this._isBetop(r.id) ? To : dt));
        let { Game: o } = this,
          s = h(o.id);
      ...
      _refreshGamepadState() {
        ...
        if (this.activeGamepadIndex === null) {
          let r = Array.from(e).find(Boolean);
          r &&
            ((this.activeGamepadIndex = r.index),
            (this.curBtnMap = this._isBetop(r.id) ? To : dt));
        }
      ...
      _resolveAction(e, r, o, s, i, d) {
        return !o || s !== 'main-menu'
          ? e
          : d - this.lastDpadTime < this.DPAD_COOLDOWN
            ? ''
            : ((this.lastDpadTime = d),
              r === 'DPAD_UP'
      ...
      _handleStandardButtons(e, r, o, s) {
        let i = this._isBetop(e.id),
          { Game: d, Store: n } = this,
          c = n.getController();
        for (let [a, l] of Object.entries(U)) {
          let u =
              !l ||
              ((r === 'replay' || r === 'game-over') && a !== 'START') ||
              (c === 'ai' &&
                r === 'playing' &&
                !x.AI_ALLOWED_ACTIONS.includes(l)),
            p = a.startsWith('DPAD_');
          if (!this._isPressed(a) || (i && p)) continue;
          if (u) return this;
          let E = this._resolveAction(l, a, p, r, o, s);
          E &&
            this.emit('dispatch:input', {
              device: 'gamepad',
              action: E,
      ...
      _collectCommands(e) {
        let { Store: r } = this,
          o = r.getState(),
          { mode: s, level: i } = o,
          d = this.activeGamepad;
        if (!d) return this;
        if (
          (this._updateActionMap(s),
          this._handleStandardButtons(d, s, i, e),
          s === 'replay' || s === 'game-over')
        )
          return this;
        let n = this._getAxis(this.AXIS_MAP.LEFT_STICK_X),
          c = this._getAxis(this.AXIS_MAP.LEFT_STICK_Y);
        if ((this._handleStickMove(n, c), this._isBetop(d.id))) {
          let a = d.axes[9] ?? 0;
          this._handleBetopDpad(a, o);
        }
        return this;
      }
      _startAxisAction(e) {
      ...
      _handleStickMove(e, r) {
        return (
          this._handleStickUp(r),
          this._handleStickDown(r),
          this._handleStickLeft(e),
          this._handleStickRight(e),
          this
        );
      }
      _getMoveUpAction(e, r) {
        let { Game: o } = this,
          s = h(o.id),
          i;
        if (e === 'main-menu') {
          let d = Number(r) + 1;
          (d >= 10 && (d = 10),
            this.emit(s.UPDATE_LEVEL, { level: d }),
            (i = `LEVEL_${So[d - 1]}`));
        } else i = 'ROTATE';
        return i;
      }
      _getMoveDownAction(e, r) {
        let o,
          { Game: s } = this,
          i = h(s.id);
        if (e === 'main-menu') {
          let d = Number(r) - 1;
          (d <= 1 && (d = 1),
            this.emit(i.UPDATE_LEVEL, { level: d }),
            (o = `LEVEL_${So[d - 1]}`));
        } else o = 'MOVE_DOWN';
        return o;
      }
      _handleBetopDpadUp(e, r, o) {
        let s = this._getMoveUpAction(e, r),
          { Game: i } = this;
        return (
          o.up ||
            ((o.up = !0),
      ...
      _handleBetopDpadDown(e, r, o) {
        let s = this._getMoveDownAction(e, r),
          { Game: i } = this;
        return (
          o.down ||
            ((o.down = !0),
      ...
      _handleBetopDpad(e, r) {
        let { mode: o, level: s } = r,
          i = e.toFixed(5),
          d = this.dpadAxisState,
          n = Date.now();
        if (o === 'main-menu' && n - this.lastDpadTime < this.DPAD_COOLDOWN)
          return this;
        switch (i) {
          case '-1.00000': {
            (this._handleBetopDpadUp(o, s, d), (this.lastDpadTime = n));
            break;
          }
          case '0.14286': {
            (this._handleBetopDpadDown(o, s, d), (this.lastDpadTime = n));
            break;
          }
          case '0.71429': {
            (this._handleBetopDpadLeft(d), (this.lastDpadTime = n));
            break;
          }
          case '-0.42857': {
            (this._handleBetopDpadRight(d), (this.lastDpadTime = n));
            break;
          }
          default: {
            d.up = d.down = d.left = d.right = !1;
            break;
          }
        }
        return this;
      ...
      _getAxis(e) {
        ...
        let r = this.activeGamepad.axes[e] ?? 0;
        return Math.abs(r) > this.DEAD_ZONE ? r : 0;
      }
      _isPressed(e) {
        let r = this.curBtnMap[e];
        if (r === void 0 || !this.activeGamepad) return !1;
        let o = this.activeGamepad.buttons[r];
        if (!o) return !1;
        let s = o.value > 0.5;
    ...
    Lo = ct;
  var Me = { holes: -5, height: -0.3, bumpiness: -0.2, completeLines: 20 },
    p2 = {
      EASY: { lookahead: 2, noise: 0.25, weights: Me, delay: 580 },
      NORMAL: { lookahead: 3, beam: 2, noise: 0.15, weights: Me, delay: 480 },
      HARD: { lookahead: 4, beam: 4, noise: 0.05, weights: Me, delay: 280 },
      EXPERT: { lookahead: 4, noise: 0, beam: 5, weights: Me, delay: 150 },
    },
    oe = p2;
  var E2 = [
  ...
  var ft = !0,
    Ao = () => {
      let t = [...lt].toSorted(() => Math.random() - 0.5);
      if (ft)
        for (; [3, 6, 7].includes(t[0].colorIndex); )
          t = [...lt].toSorted(() => Math.random() - 0.5);
      return ((ft = !1), t);
    };
  Ao._reset = () => {
    ft = !0;
  ...
  var Ge = [],
    v2 = (t = 1) => {
      Ge.length === 0 && (Ge = Ro());
      let e = Ge.pop(),
        r = Math.min(Math.floor((t - 1) / 32), at.length - 1),
        o = at[r];
      return {
  ...
  var _2 = (t) =>
      structuredClone({
        ...
        lines: t.lines,
        combo: t.combo || 0,
        backToBack: t.backToBack || !1,
        tSpin: t.tSpin || null,
        cur: t.curr,
        next: t.next,
        piece: t.curr
          ? { shape: t.curr.shape, position: { x: t.cx, y: t.cy } }
          : null,
        mode: t.mode,
        bag: Co(),
        hold: t.hold || null,
  ...
  var g2 = (t) => {
      let e = t.length,
        r = t[0].length,
        o = Array.from({ length: r }, () => Array.from({ length: e }).fill(0));
      for (let s = 0; s < e; s += 1)
        for (let i = 0; i < r; i += 1) o[i][e - s - 1] = t[s][i];
      return o;
    },
    Oo = g2;
  var S2 = (t, e) => {
  ...
  var T2 = (t, e, r, o) => {
      for (let s = 0; s < e.length; s += 1)
        for (let i = 0; i < e[s].length; i += 1) {
          ...
          let d = r + i,
            n = o + s;
          if (d < 0 || d >= t[0].length || n >= t.length || (n >= 0 && t[n][d]))
            return !0;
        }
      return !1;
    },
  ...
  var L2 = (t, e, r) => {
    ...
    Go = L2;
  var A2 = (t, e) => {
      for (let r = 0; r < e; r++) t.push('ROTATE');
    },
    Do = A2;
  var R2 = (t, e) => {
      if (e === 0) return;
      let r = e > 0 ? 'MOVE_RIGHT' : 'MOVE_LEFT',
        o = Math.abs(e);
      for (let s = 0; s < o; s++) t.push(r);
    },
    wo = R2;
  var C2 = ({ rotationCount: t, targetX: e, originalX: r }) => {
      let o = [];
      return (Do(o, t), wo(o, e - r), o.push('DROP'), o);
    },
    Po = C2;
  var x2 = ({
      board: t,
      ...
      let { y: i, placeOn: d } = Go(t, e, r),
        n = Po({ rotationCount: s, targetX: r, originalX: o.position.x });
      return { y: i, placeOn: d, actions: n };
    },
    Io = x2;
  var O2 = (t, e, r = !1) => {
      let o = [],
      ...
      for (let n = 0; n < d; n++) {
        ...
        for (let a of c) {
          ...
          (r && (l.actions = ['HOLD', ...l.actions]), o.push(l));
        }
        s = Oo(s);
      }
      return o;
    ...
    ht = O2;
  var y2 = (t) => {
      let { board: e, piece: r, hold: o, next: s } = t,
        i = ht(e, r, !1),
        d = o || s;
      if (d) {
        let n = {
          shape: d.shape,
          position: {
            x: Math.floor(e[0].length / 2) - Math.floor(d.shape[0].length / 2),
            y: 0,
          },
        };
        i.push(...ht(e, n, !0));
    ...
    bo = y2;
  var M2 = (t, e) => {
      for (let r = 0; r < t.length; r++) if (t[r][e]) return t.length - r;
      return 0;
    },
    Bo = M2;
  var G2 = (t) => {
  ...
  var D2 = (t, e, r) => {
      ...
      for (let T = 0; T < t[0].length; T++) o.push(Bo(t, T));
      let i = o.reduce((T, A) => T + A, 0),
        d = Math.max(...o),
        n = 0;
      for (let T = 0; T < o.length - 1; T++) n += Math.abs(o[T] - o[T + 1]);
      let c = No(t),
        a = 0;
      d > 10 && (a = -Math.pow(d - 10, 2) * 1);
      let l = [0, 1, 4, 8, 20, 30],
        u = r ? r.cleared : 0,
        p = l[u] || 0,
        E =
      ...
      return (
        r &&
          ((m += r.clearScore * 0.01),
          r.isTSpin ? (m += 5) : r.isTSpinMini && (m += 2),
          r.isBackToBack && (m += 3),
          r.isAllClear && (m += 10),
          (m += r.combo * 0.5)),
        E + m
  ...
  var P2 = (t, e, r, o) => {
      ...
      for (let i = 0; i < e.length; i++)
        for (let d = 0; d < e[0].length; d++) {
          ...
          let n = d + r,
            c = i + o;
          c >= 0 && c < s.length && (s[c][n] = 1);
        }
      return s;
    },
    ko = P2;
  var I2 = (t) => {
      let e = t.filter((r) => !r.every((o) => o !== 0));
      for (; e.length < t.length; )
        e.unshift(Array.from({ length: t[0].length }).fill(0));
      return e;
    },
    fe = I2;
  var b2 = (t, e, r) =>
      e ? [400, 800, 1200, 1600][t] || 0 : (r && [100, 200, 400][t]) || 0,
    De = b2;
  var B2 = (t, e) => {
      let { CLEAR_LINE_SCORES: r } = x,
        o = t.filter((A) => A.every((R) => R !== 0)).length,
        { isTSpin: s = !1, isTSpinMini: i = !1 } = e.tSpin || {};
      if (o === 0 && !s && !i) return null;
      let n = De(o, s, i) || r[o] || 0,
        c = o >= 4 || s || i,
        a = c && e.backToBack === !0,
        l = a ? 1.5 : 1,
        u = (e.combo || 0) + 1,
        p = u > 1 ? (u - 1) * 50 : 0,
        E = o > 0 && t.every((A) => A.every((R) => R === 0)),
        m = E ? 2e3 : 0,
        T = Math.floor(n * l) + p + m;
      return {
        cleared: o,
        baseScore: n,
        clearScore: T,
    ...
    se = B2;
  var N2 = (t, e) => {
      let r = ko(t.board, t.piece.shape, t.piece.position.x, e.y),
        o = fe(r),
        s = se(o, t),
        i = t.bag ? [...t.bag] : [],
        d =
      ...
      let c = {
        shape: d.shape,
        position: {
          x: Math.floor(10 / 2) - Math.floor(d.shape[0].length / 2),
          y: 0,
        },
      };
      return {
        ...
        bag: i,
        combo: s ? s.combo : 0,
        backToBack: s ? s.isBigMove : t.backToBack,
        tSpin: null,
        clearResult: s || null,
      };
    },
      ...
      if (r > 1 && s.length > o) {
        let n = s.map((c) => {
          ...
          let l = fe(a),
            ...
            p = ae(l, e, u);
          return (
            c.actions.includes('HOLD') && (p += 2),
            { move: c, score: p }
          );
        });
        (n.sort((c, a) => a.score - c.score),
          (s.length = 0),
          s.push(...n.slice(0, o).map((c) => c.move)));
      }
      let i = null,
        d = -1 / 0;
      for (let n of s) {
        ...
        (n.actions.includes('HOLD') && (u += 2), u > d && ((d = u), (i = n)));
      }
      return i;
    },
    zo = Uo;
  var mt = class extends v {
      ...
      loop = () => {
        ...
        let { Game: e, Animations: r, Scheduler: o } = this,
          s = e.Store.getState();
        if (s.mode !== 'playing' || r.hasBlocking()) {
          this.aiSchedulerId = o.delay(this.loop, 100);
          return;
        }
        let i = this.getDifficultyConfig();
        if (this.actions.length === 0 && !this.workerBusy) {
          let n = this.think(s, i);
          n && (this.actions = [...n.actions]);
        }
        let d = this.actions.shift();
        if (!d && this.workerBusy) {
      ...
      think(e, r) {
        let { Store: o } = this,
          { lookahead: s, weights: i, beam: d } = r,
      ...
      _onWorkerMessage = (e) => {
        let { type: r, best: o, error: s } = e.data;
        (r === 'result' &&
      ...
      subscribe() {
        let { Game: e } = this,
          r = H(e.id);
        (this.on(r.START, this._onStart), this.on(r.STOP, this._onStop));
      }
      unsubscribe() {
        let { Game: e } = this,
          r = H(e.id);
        (this.off(r.START, this._onStart), this.off(r.STOP, this._onStop));
      }
      _onStart = () => this.start();
      _onStop = () => this.stop();
    },
  ...
  var k2 = {
    ...
    pt = class extends v {
      ...
      initialize() {
        let { Controls: e } = this;
        ((this.level = 0),
          (this.$back = document.querySelector(`#${e.back}`)),
          (this.$hold = document.querySelector(`#${e.hold}`)),
          (this.$start = document.querySelector(`#${e.start}`)),
          (this.$up = document.querySelector(`#${e.up}`)),
          (this.$down = document.querySelector(`#${e.down}`)),
          (this.$left = document.querySelector(`#${e.left}`)),
          (this.$right = document.querySelector(`#${e.right}`)),
          (this.$a = document.querySelector(`#${e.a}`)),
          (this.$b = document.querySelector(`#${e.b}`)),
          (this.$x = document.querySelector(`#${e.x}`)),
          (this.$y = document.querySelector(`#${e.y}`)),
  ...
  var Et = class extends v {
      ...
      subscribe() {
        let { Game: e } = this,
          r = C(e.id);
        (this.on(r.START_RECORD, this._onStartRecord),
          this.on(r.STOP_RECORD, this._onStopRecord),
          this.on(r.ADD_RECORD, this._onAddRecord),
          this.on(r.ADD_PIECE, this._onAddPiece),
          this.on(r.START_PLAY, this._onStartPlay),
          this.on(r.RESET, this._onReset),
          this.on(r.GAME_OVER, this._onGameOver),
          this.on(r.STOP_CLEAR_LINES, this._onStopClearLines));
      }
      unsubscribe() {
        let { Game: e } = this,
          r = C(e.id);
        (this.off(r.START_RECORD, this._onStartRecord),
          this.off(r.STOP_RECORD, this._onStopRecord),
          this.off(r.ADD_RECORD, this._onAddRecord),
          this.off(r.ADD_PIECE, this._onAddPiece),
          this.off(r.START_PLAY, this._onStartPlay),
          this.off(r.RESET, this._onReset),
          this.off(r.GAME_OVER, this._onGameOver),
          this.off(r.STOP_CLEAR_LINES, this._onStopClearLines));
      }
      _onStartRecord = () => {
        let { Replay: e } = this;
        e.startRecord();
      ...
      _onGameOver = () => {
        ...
        e.hasData
          ...
          : (this.emit(d.UPDATE_MODE, { mode: 'game-over' }),
            this.emit(i.UPDATE_MODE, { mode: 'game-over' }));
      };
      _onStopClearLines = ({ isLevelUp: e, level: r }) => {
        ...
        (this.emit(i.STOP_BGM),
          ...
          this.emit(d.START_LEVEL_UP, { level: r }));
      };
    },
    $o = Et;
  var qt = class extends v {
      ...
      initialize() {
        ((this.recording = !1),
          ...
          (this.timestamp = 0));
        let { Game: e } = this;
        this.Router = new $o({ Replay: this, Game: e });
      }
      getNextPiece() {
        if (!this.playing) return { curr: null, next: null };
        let e = this.pieceSequence[this.pieceIndex++];
        if (!e) return { curr: null, next: null };
        let r = this.pieceSequence[this.pieceIndex] || null;
        return { curr: e, next: r };
      }
      syncPlayElapsed({ timestamp: e, isBlocked: r }) {
      ...
      update({ speed: e, timestamp: r }) {
        let { Store: o, Game: s, data: i } = this,
          d = o.getMode();
        if (((this.timestamp = r), !this.playing || d !== 'replay')) return;
        let n = h(s.id);
        if (i.length > 0 && this.cursor >= i.length) {
          (this.stopPlay(), this.emit(n.UPDATE_MODE, { mode: 'game-over' }));
          return;
        }
        let c = i[this.cursor];
        if (c) {
          let a = e ?? 1e3,
            l = c.ms - this.playElapsed;
          if (l > a * 2) {
            let u = Math.min(l - a, 1e3);
            ((this.playElapsed += u), (this.startTime = r - this.playElapsed));
          }
        }
        for (
          ;
          ...
          let { cmd: a } = i[this.cursor];
          if (a.action === 'HOLD_SYNC') return;
          (this.emit('dispatch:command', a), this.cursor++);
        }
      }
      startRecord() {
        ((this.recording = !0),
  ...
  var vt = class extends v {
      ...
      initialize() {
        ...
        let { Scheduler: e } = this,
          r = g();
        (this.emit(r.PLAY_SOUND, { sound: 'COUNTDOWN' }),
      ...
      render() {
        let { Game: e } = this,
          r = _(e.id);
        this.emit(r.RENDER_COUNTDOWN, { state: this.state });
      }
    },
    Ko = vt;
  var _t = class extends v {
      ...
      _startTick() {
        ...
        this._tickId = e.interval(() => {
          this.emit(r.PLAY_SOUND, { sound: 'SECOND_TICK' });
        }, 1e3);
      }
      resume() {
  ...
  var U2 = (t, e) => {
      let r = 1,
        o = 10,
        s = 0;
      for (; r < e && t >= s + o; ) ((s += o), r++, (o = Math.min(o + 2, 60)));
      return { level: r, levelUpSteps: o };
    },
    Jo = U2;
  var z2 = (t) => Array.from({ length: t }).fill(0),
    V2 = (t) => {
      let { MAX_LEVEL: e, CLEAR_LINE_SCORES: r, ALL_CLEAR_SCORE: o = 2e3 } = x,
        { Elements: s, Store: i } = t,
        d = i.getState(),
        { cols: n } = s.Canvas,
        c = [...(d.clearLines || [])].toSorted((k, Ue) => Ue - k),
        a = c.length,
        l = structuredClone(d.board);
      for (let k of c) l.splice(k, 1);
      for (let k = 0; k < a; k++) l.unshift(z2(n));
      let u = d.lines + a,
        p = d.baseLines + u,
        { level: E, levelUpSteps: m } = Jo(p, e),
        T = E > d.level,
        { isTSpin: A = !1, isTSpinMini: R = !1 } = d.tSpin || {},
        w = De(a, A, R),
        X = a >= 4 || A || R,
        j = X && d.backToBack === !0,
        H0 = j ? 1.5 : 1,
        U0 = X,
        Ct = a > 0 && l.every((k) => k.every((Ue) => Ue === 0)),
        z0 = Ct ? o : 0,
        V0 = w || r[a] || 0,
        ke = Math.floor(V0 * H0 * E),
        he = a > 0 ? (d.combo || 0) + 1 : 0,
        He = he > 1 ? (he - 1) * 50 : 0;
      return {
        stateHandler: (k) => ({
          ...k,
  ...
  var gt = class extends v {
      ...
      initialize(e) {
        ...
        let { clearScore: c, combo: a, comboScore: l } = ue(s),
          u = () => {
      ...
      dispose() {
        let { Scheduler: e, Game: r } = this;
        for (let a of this._schedulerIds) e.cancel(a);
        let o = r.id,
          s = ue(r),
          { level: i, levelUp: d } = s,
          n = h(o),
        ...
        e.sequence([
          {
            fn: () => {
              this.emit(c.STOP_CLEAR_LINES, { isLevelUp: d, level: i });
            },
          },
          {
            fn: () => {
              this.emit(n.UPDATE_STATE, { stateHandler: s.stateHandler });
            },
          },
          {
            fn: () => {
              this.emit(n.SAVE_HIGH_SCORE);
            },
          },
          {
            fn: () => {
              this.emit(n.UPDATE_HUD);
            },
          },
        ]);
      }
      render() {
        let { Game: e } = this,
          r = _(e.id);
        this.emit(r.RENDER_CLEAR_LINES, { state: { lines: this.lines } });
      }
    },
    Qo = gt;
  var St = class extends v {
      ...
      initialize() {
        let { scoreData: e, Scheduler: r } = this,
          { score: o, lines: s, combo: i, comboScore: d } = e;
        ((this.layer = 300),
          (this.blocking = !1),
  ...
  var Tt = class extends v {
      ...
      createFireworks() {
        let { UI: e } = this,
          { width: r, height: o } = e.Renderer.Canvas.gameBoard,
      ...
      dispose() {
        ...
        let r = g();
        this.emit(r.RESUME_BGM, { level: this.level });
      }
      render() {
        let { Game: e } = this,
          r = _(e.id);
        this.emit(r.RENDER_LEVEL_UP, {
          level: this.level,
  ...
  var Lt = class extends v {
      ...
      initialize() {
        ...
        for (let n = 0; n < o.length; n++)
          for (let c = 0; c < o[n].length; c++)
            o[n][c] && d.push({ x: s + c, y: i + n });
        ((this.state = { cells: d }),
          (this._endId = r.delay(() => {
            this._finished = !0;
          }, 150)));
    ...
    e0 = Lt;
  var F2 = (t) => {
      let { Replay: e, Store: r } = t;
      if (e.playing) return e.getNextPiece();
      let o = r.getState(),
        { next: s, level: i } = o;
      return {
        curr: s ? { ...s, shape: s.shape.map((n) => [...n]) } : ut(i),
        next: ut(i),
      };
    },
    t0 = F2;
  var W2 = (t, e, r, o) => {
      let { Elements: s, Store: i } = t,
        { rows: d, cols: n } = s.Canvas,
        c = i.getState(),
        { curr: a, cx: l, cy: u, board: p } = c;
      if (!a) return !1;
      let E = o || a.shape;
  ...
  var $2 = (t) => {
      ...
      let s = g(),
        i = C(e);
      (t.emit(i.STOP_RECORD),
        t.emit(s.STOP_BGM),
        t.emit(s.PLAY_SOUND, { sound: 'GAME_OVER' }),
        t.emit(i.GAME_OVER));
    },
    we = $2;
  var Y2 = (t) => {
      let { id: e, Elements: r, Store: o } = t,
        { cols: s } = r.Canvas,
        { curr: i, next: d } = t0(t);
      if (!i) return;
      o.setState({
      ...
      let c = _(e),
        a = C(e);
      (t.emit(c.RENDER_NEXT_PIECE, { state: n }), t.emit(a.ADD_PIECE, n.curr));
    },
    N = Y2;
  var K2 = (t, e, r = 1) => {
      let { Store: o } = t;
    ...
    ie = K2;
  var X2 = (t) => {
      let { Store: e, id: r, Scheduler: o } = t,
        s = g(),
        i = C(r),
        d = _(r),
        n = e.getLevel();
      (t.emit(i.START_RECORD),
        e.resetBoard(),
        ie(t, 'playing', n),
        t.emit(d.UPDATE_HUD),
        N(t),
        t.emit(s.PLAY_SOUND, { sound: 'GAME_STARTED' }),
        o.delay(() => {
          t.emit(s.RESUME_BGM, { level: n });
        }, 250));
    },
    r0 = X2;
  var J2 = (t) => {
  ...
  var Q2 = (t) => {
      let { id: e, Store: r } = t,
        ...
        d = _(e);
      o === 'playing' &&
        (t.emit(d.UPDATE_MODE, { mode: 'paused' }),
        r.setMode('paused'),
        t.emit(s.STOP_BGM),
        t.emit(s.PLAY_SOUND, { sound: 'PAUSED' }),
        t.emit(i.START_PAUSED));
    },
    s0 = Q2;
  var j2 = (t) => {
      let { id: e, Store: r } = t;
      if (r.getMode() !== 'paused') return;
      let s = r.getLevel(),
        i = g(),
        d = h(e),
        n = _(e);
      (t.emit(n.UPDATE_MODE, { mode: 'playing' }),
        r.setMode('playing'),
        t.emit(d.STOP_PAUSED),
        t.emit(i.PLAY_SOUND, { sound: 'RESUME' }),
        t.emit(i.RESUME_BGM, { level: s }));
    },
    i0 = j2;
  var Z2 = (t) => {
      let e = t.Store.getMode();
      e === 'main-menu' ||
        e === 'replay' ||
        e === 'game-over' ||
        (e === 'playing' ? s0(t) : i0(t));
  ...
  var e5 = (t, e = 'main-menu') => {
      ...
      (t.emit(s.STOP_BGM),
        ...
        e === 'main-menu' &&
          ...
          (l = 1),
          t.emit(s.PLAY_SOUND, { sound: 'SWITCH_SCENE' })),
        ie(t, e, l),
        t.emit(a.UPDATE_HUD, { state: o.getState() }),
        t.emit(a.UPDATE_MODE, { mode: e }),
        t.emit(a.CLEAR_NEXT_PIECE),
        t.emit(a.CLEAR_HOLD_PIECE),
        t.emit(i.STOP),
        o.setController('human'),
        t.emit(a.UPDATE_CONTROLLER, { controller: 'human' }),
        t.emit(c.RESET));
    },
    Pe = e5;
  var t5 = (t) => {
  ...
  var o5 = (t, e, r, o = !1) => {
      ...
      if (!W(t, e, r)) {
        ((n += e),
          (c += r),
          s.setState({ cx: n, cy: c }),
          r > 0 && !o && s.setState({ score: s.getScore() + 1 }));
        let { curr: a } = s.getState();
        return (
          a._lockTimer && (a._lockTimer = 0),
          t.emit(d.PLAY_SOUND, { sound: 'MOVE' }),
          !0
        );
      }
      return !1;
  ...
  var s5 = (t) => (t === 'I' ? a0 : t === 'I5' ? l0 : t === 'O' ? null : f0),
    u0 = s5;
  var i5 = (t, e) => ((t ?? 0) + e + 4) % 4,
    h0 = i5;
  var n5 = (t) => {
      let e = t.length,
        r = t[0].length,
      ...
      for (let s = 0; s < e; s++)
        for (let i = 0; i < r; i++) o[r - 1 - i][s] = t[s][i];
      return o;
    },
    m0 = n5;
  var d5 = (t) => {
      let e = t.length,
        ...
        o = Array.from({ length: r })
          ...
          .map(() => Array.from({ length: e }).fill(0));
      for (let s = 0; s < e; s++)
        for (let i = 0; i < r; i++) o[i][e - 1 - s] = t[s][i];
      return o;
    },
    p0 = d5;
  var c5 = (t, e) => (e === 1 ? p0(t) : m0(t)),
    E0 = c5;
  var a5 = (t, e, r, o, s, i) => {
      let d = { curr: { ...e, shape: r, rotation: o } };
      (s !== void 0 && (d.cx = s), i !== void 0 && (d.cy = i), t.setState(d));
    },
    Ie = a5;
  var l5 = (t) => {
      let { curr: e } = t.Store.getState();
      e?._lockTimer && (e._lockTimer = 0);
    },
    be = l5;
  var f5 = (t, e, r, o, s) => {
      let { cx: i, cy: d } = t.Store.getState();
      for (let [n, c] of s) {
        let a = n,
          l = -c;
        if (!W(t, a, l, r))
          return (Ie(t.Store, e, r, o, i + a, d + l), be(t), !0);
      }
      return !1;
    },
    q0 = f5;
  var u5 = (t, e, r, o) =>
      W(t, 0, 0, r) ? !1 : (Ie(t.Store, e, r, o), be(t), !0),
    v0 = u5;
  var h5 = (t, e = 1) => {
      let { Store: r } = t,
        { curr: o } = r.getState();
      ...
      let s = E0(o.shape, e),
        ...
        n = g();
      if (d?.length) {
        let c = d[(o.rotation ?? 0) % 4];
        if (c?.length && q0(t, o, s, i, c)) {
          ((o._lastAction = 'rotate'),
            t.emit(n.PLAY_SOUND, { sound: 'ROTATE' }));
          return;
        }
      }
      v0(t, o, s, i) &&
        ((o._lastAction = 'rotate'), t.emit(n.PLAY_SOUND, { sound: 'ROTATE' }));
    },
    _0 = h5;
  var m5 = (t) => {
      let { Store: e, Elements: r } = t,
        o = e.getState(),
        { curr: s, cx: i, cy: d, board: n } = o,
        { rows: c, cols: a } = r.Canvas;
  ...
  var p5 = (t) => {
      ...
      if (!o) return;
      let s = o.shape,
        i = structuredClone(r.board);
      for (let n = 0; n < s.length; n++)
        for (let c = 0; c < s[n].length; c++)
          if (s[n][c]) {
            let a = r.cy + n,
              l = r.cx + c;
            if (a < 0 || a >= i.length || l < 0 || l >= i[0].length) continue;
            i[a][l] = o.color;
          }
      let d = g0(t);
      (e.setState({ board: i, tSpin: d }), (o._lastAction = null));
    },
    Be = p5;
  var E5 = (t) => {
      let { Elements: e, Store: r } = t,
        o = r.getState(),
        { rows: s } = e.Canvas,
        i = [];
      for (let d = s - 1; d >= 0; d--)
        o.board[d].every((c) => !!c) && i.push(d);
      return i;
    },
    S0 = E5;
  var q5 = (t) => {
      ...
      if (o.length === 0) {
        let i = _(e),
          d = { combo: 0 };
        (r.setState(d), t.emit(i.UPDATE_HUD, d));
        return;
      }
      r.setClearLines(o);
      let s = h(e);
      t.emit(s.START_CLEAR_LINES, { linesToClear: o });
    },
    Ne = q5;
  var v5 = 300,
    _5 = (t, e) => {
      let r = t.Store.getMode();
      if ((r !== 'playing' && r !== 'replay') || e) return;
      r === 'playing' &&
        t.emit('dispatch:input', {
          device: 'replay',
          action: 'AUTO_TICK',
          payload: { Game: t },
        });
      let o = g(),
        s = h(t.id),
        { curr: i, cx: d, cy: n } = t.Store.getState();
      if (ne(t, 0, 1)) {
        i._lockTimer && (i._lockTimer = 0);
        return;
      }
      (i._lockTimer || (i._lockTimer = 0),
        (i._lockTimer += t.getSpeed()),
        i._lockTimer >= v5 &&
          ((i._lockTimer = 0),
          Be(t),
          t.emit(s.START_LANDING_FLASH, {
            piece: { shape: i.shape, cx: d, cy: n },
          }),
          t.emit(o.PLAY_SOUND, { sound: 'FALL' }),
          Ne(t),
          N(t)));
    },
    T0 = _5;
  var g5 = (t) => {
      ...
      let d = g(),
        n = h(t.id),
        { curr: c, cx: a, cy: l } = t.Store.getState();
      (Be(t),
        t.emit(n.START_LANDING_FLASH, {
          piece: { shape: c.shape, cx: a, cy: l },
        }),
        t.emit(d.PLAY_SOUND, { sound: 'FALL' }),
        Ne(t),
        N(t),
        t.emit(d.PLAY_SOUND, { sound: 'DROP' }));
    },
    L0 = g5;
  var S5 = (t) => {
      let { Store: e, Elements: r, id: o } = t,
        s = e.getState(),
        { curr: i, hold: d } = s,
        { cols: n } = r.Canvas;
      if (!i || i._held) return;
      if (d) {
        ...
        e.setState({
          curr: a,
          hold: l,
          cx: Math.floor(n / 2) - Math.floor(a.shape[0].length / 2),
          cy: 0,
        });
      } else (e.setState({ hold: { ...i, _held: !0 } }), N(t));
      let c = _(o);
      t.emit(c.RENDER_HOLD_PIECE);
    },
    A0 = S5;
  var T5 = (t) => {
      let { MAX_LEVEL: e } = x,
    ...
    R0 = T5;
  var L5 = (t) => localStorage.getItem(t),
    C0 = L5;
  var A5 = (t, e) => {
  ...
  var At = class extends v {
      ...
      initialize() {
        let { Elements: e, Block: r, Scheduler: o } = this,
          { Controls: s } = e,
          i = new rr({ ...e.Canvas, GameState: Zt });
        ((this.id =
          crypto?.randomUUID?.() ||
          `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`),
          (this.effect = null),
          (this.Store = i),
          (this.Animations = new sr({ Game: this })),
          (this.CommandQueue = new or({ Game: this })),
          (this.AI = new Vo({
            Game: this,
            Store: i,
            Scheduler: o,
          ...
          (this.UI = new qo({ Game: this, Store: i, Elements: e, Block: r })),
          (this.Keyboard = new go({ Game: this, Store: i })),
          (this.Gamepad = new Lo({ Game: this, Store: i })),
          (this.Touch = new Wo({ Game: this, Store: i, Controls: s })),
          (this.Replay = new Yo({ Game: this, Store: i, Scheduler: o })),
          (this.Router = new jt({
            Animations: this.Animations,
            AI: this.AI,
      ...
      selectLevel(e) {
        ...
        (this.Store.setLevel(e),
          this.emit(r.PLAY_SOUND, { sound: 'LEVEL_CHANGED' }));
      }
      switchToDifficulty() {
        let e = g();
      ...
      selectDifficulty(e) {
        ...
        (this.Store.setDifficulty(e),
          this.emit(r.PLAY_SOUND, { sound: 'DIFFICULTY_CHANGED' }));
      }
      switchToMainMenu() {
        let e = g(),
          r = _(this.id);
        (this.emit(r.UPDATE_MODE, { mode: 'main-menu' }),
          this.Store.setMode('main-menu'),
          this.emit(e.PLAY_SOUND, { sound: 'SWITCH_SCENE' }));
      }
      loadHighScore() {
        let e = Number.parseInt(C0('tetris-high-score'), 10) || 0;
        this.Store.setHighScore(e);
      }
      saveHighScore(e) {
        let { Store: r } = this;
        e > r.getHighScore() &&
          (r.setHighScore(e), x0('tetris-high-score', e.toString()));
      }
      begin() {
        r0(this);
      }
      ...
      getGhostPosition(e) {
        ...
        r &&
          r.cy !== e.cy &&
          this.emit(o.RENDER_GHOST_PIECE, { ghost: { ...e, ...r } });
      }
      spawn() {
        N(this);
      }
      ...
      startCountdown() {
        let { Scheduler: e } = this;
        this.Animations.register(new Ko({ Scheduler: e, Game: this }));
      }
      startPaused() {
        let { Scheduler: e } = this;
        ((this.effect = new Xo({ Scheduler: e })),
      ...
      startClearLines(e) {
        let { Scheduler: r, Store: o } = this;
        this.Animations.register(
          new Qo({ Game: this, Store: o, Scheduler: r, lines: e }),
        );
      }
      startClearScore(e) {
        let { Scheduler: r } = this;
        this.Animations.register(
          new jo({ Game: this, scoreData: e, Scheduler: r }),
        );
      }
      startLevelUp(e) {
        let { Scheduler: r, UI: o } = this;
        this.Animations.register(
          new Zo({ Game: this, UI: o, level: e, Scheduler: r }),
        );
      }
      startLandingFlash(e) {
        let { Scheduler: r } = this;
        this.Animations.register(
          new e0({ Game: this, piece: e, Scheduler: r }),
        );
      }
      addEventListeners() {
  ...
  var Rt = class extends v {
      constructor(e, r) {
        (super(r), this.initialize(e, r));
      }
      initialize(e, r = {}) {
        ((this.action = e), (this.payload = r));
      }
      execute() {
        let { action: e, payload: r } = this;
        this.emit('dispatch:command', { action: e, payload: r });
      }
    },
    y0 = Rt;
  var R5 = (t, e) => {
      let { action: r, payload: o } = t,
        { isBlocked: s, ms: i } = e;
      if (s || !r) return;
      let { Game: d } = o,
        n = new y0(r, o),
        ...
        a = Z(c),
        l = C(c);
      (d.emit(a.ENQUEUE, { cmd: n }), d.emit(l.ADD_RECORD, { ms: i, cmd: n }));
    },
    M0 = R5;
  var C5 = {
      LEVEL_ONE: (t) => {
        ...
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 1 });
      },
      LEVEL_TWO: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 2 });
      },
      LEVEL_THREE: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 3 });
      },
      LEVEL_FOUR: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 4 });
      },
      LEVEL_FIVE: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 5 });
      },
      LEVEL_SIX: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 6 });
      },
      LEVEL_SEVEN: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 7 });
      },
      LEVEL_EIGHT: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 8 });
      },
      LEVEL_NINE: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 9 });
      },
      LEVEL_TEN: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_LEVEL, { level: 10 });
      },
      CONFIRM: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SWITCH_TO_DIFFICULTY);
      },
    },
    G0 = C5;
  var x5 = {
      EASY: (t) => {
        ...
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_DIFFICULTY, { difficulty: 'easy' });
      },
      NORMAL: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_DIFFICULTY, { difficulty: 'normal' });
      },
      HARD: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_DIFFICULTY, { difficulty: 'hard' });
      },
      EXPERT: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SELECT_DIFFICULTY, { difficulty: 'expert' });
      },
      BACK: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.SWITCH_TO_MAIN_MENU);
      },
      CONFIRM: (t) => {
        let e = t?.Game;
        if (!e) return;
  ...
  var O5 = {
      ...
      MOVE_DOWN: (t) => {
        ...
        if (!e) return;
        let r = h(e.id);
        e.emit(r.BLOCK_MOVE, { ox: 0, oy: 1 });
      },
      ROTATE: (t) => {
        let e = t?.Game;
        if (!e) return;
        let r = h(e.id);
        e.emit(r.BLOCK_ROTATE);
      },
      DROP: (t) => {
        let e = t?.Game;
        if (!e) return;
  ...
  var q = {
      ...
      initialize: (t) => {
        q.Scheduler = new Ot();
        let e = { ...t, Scheduler: q.Scheduler, isAIPlayer: !0 };
        ((q.Audio = new Qt(e)), (q.Game = new O0(e)));
      },
      launch: () => {
        q.initialize(q.Configuration);
        let { Game: t } = q,
          { Store: e, UI: r } = t;
        (e.resetBoard(),
          t.loadHighScore(),
          t.setBeginningState('main-menu'),
          r.updateMode('main-menu'),
          r.resize(),
          r.updateHud(),
          r.updateController(e.getController()),
          r.lazyRender(),
          q.subscribe(),
          t.addEventListeners(),
          q.start());
      ...
      tick: (t) => {
        q.lastTickTime || ((q.lastTickTime = t), (q.fixedAccumulator = t));
        let { Game: e, Scheduler: r } = q,
          {
            UI: o,
            Replay: s,
            Gamepad: i,
          ...
          a = n.hasBlocking(),
          l = t - q.fixedAccumulator;
        ((q.lastTickTime = t),
          r.tick(t),
          s.syncPlayElapsed({ timestamp: q.lastTickTime, isBlocked: a }),
          s.update({ speed: e.getSpeed(), timestamp: q.lastTickTime }),
          i.update(t),
          d.update(),
          c.flush(),
          (!q.fixedAccumulator || l > e.getSpeed()) &&
            !s.playing &&
            (e.tick(a), (q.fixedAccumulator = t)),
          n.flush(),
          o.tickHud(),
          o.render(),
          n.render(),
          (q.rafId = requestAnimationFrame(q.tick)));
      },
      subscribe: () => {
        let { Game: t, Audio: e } = q;
        (q._subscribe(), e?.subscribe?.(), t?.subscribe?.());
      },
      unsubscribe: () => {
        let { Game: t, Audio: e } = q;
        (q._unsubscribe(), e?.unsubscribe?.(), t?.unsubscribe?.());
      },
      _subscribe: () => {
        let { Game: t } = q;
        (t.on('dispatch:command', q._onDispatchCommand),
          t.on('dispatch:input', q._onDispatchInput));
      },
      _unsubscribe: () => {
        let { Game: t } = q;
        t &&
          (t.off('dispatch:command', q._onDispatchCommand),
          t.off('dispatch:input', q._onDispatchInput));
      },
      _onDispatchCommand: (t) => {
        let { Game: e } = q,
          { Animations: r, Store: o } = e,
          s = o.getMode(),
          i = r.hasBlocking(['clear-lines', 'countdown', 'level-up']),
          { payload: d } = t;
        ((d.isBlocked = i), B0(t, { mode: s }));
      },
      _onDispatchInput: (t) => {
        let { Game: e } = q,
          { Animations: r, Replay: o } = e,
          s = r.hasBlocking(['clear-lines', 'countdown', 'level-up']),
          i = q.lastTickTime - o.startTime;
        M0(t, { isBlocked: s, ms: i });
      },
      start: () => {
        q.rafId = requestAnimationFrame(q.tick);
      },
      stop: () => {
        q.rafId &&
          (cancelAnimationFrame(q.rafId),
          (q.rafId = 0),
          (q.lastTickTime = 0),
          (q.fixedAccumulator = 0));
      },
      restart: () => {
        (q.stop(), q.start());
      },
