...
首先需要特表说明一下 tetris.js 的 AI 决策的核心算法是：**Beam Search**（减枝）。

当然，我也查阅过一些其他常用的算法，也尝试过**MCTS（蒙特卡洛树搜索）**算法。但由于 tetris.js 的定位是希望又纯 JavaScript 实现，在浏览器环境 MCTS 算法的计算量太大了，即便是标准的 10 x 20 的棋盘计算量也很大，很容易就导致占用内存过高浏览器卡死。
