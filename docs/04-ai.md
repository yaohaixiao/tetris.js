...
当然，我也查阅过一些其他常用的算法，也尝试过
**MCTS**（蒙特卡洛树搜索）算法。但由于 tetris.js 的定位是希望用纯 JavaScript 实现，在浏览器环境 MCTS 算法的计算量太大了，即便是标准的 10
x 20 的棋盘计算量也很大，很容易就导致占用内存过高浏览器卡死。
...
7-bag 算法的可预测性结合 Beam
Search 算法实现的效果非常符合我的预期，AI 决策的效果和性能都不错，所有 tetris.js 选择了
**Beam Search**。
...
每一次决策，AI 都会复制当前状态，也就是参数中的
`snapshot`，然后在独立环境中不断模拟未来，整个过程不会影响真实游戏。

### createSnapshot 创建真实棋盘的快照

不能直接修改游戏棋盘的数据，`snapshot` 是“拷贝”一份棋盘数据，使用
**`createSnapshot`** 方法创建一个真实棋盘的快照：
...
`generateForPiece`
方法遍历方块的旋转状态和所有合法水平位置，对每个组合模拟硬降到底（获取终点 Y 坐标），并生成动作序列（旋转 → 平移 → 硬降）：
...
这些可能性组也就是计算出来的 **moves**
成了一棵搜索树，后续所有计算都会建立在这些合法操作之上。只有真正能够落地的位置，才会进入下一阶段。
...
整个过程都发生在模拟环境，因为又再次 `cloneBoard(snapshot.board)`
克隆了棋盘的数据，真实游戏不会受到任何影响。
...
AI 决策的关键一个依据就是操作的结果是否能够实现消除行，而这个计算则是由
`simulateClearResult` 函数处理的：
...
Simulation 的结果是 AI 的后续 **Evaluation**
评分的基础，也是后续所有评分的依据。
...
- Column
  Transitions（列方向变化）：每列中空格→方块或方块→空格的切换次数，反映棋盘的"破碎"程度；
- Well
  Depth（井深）：两侧被堵死的深槽深度。I 块需要直槽才能消 4 行，AI 需要有意识地留"井"；

#### 消行指标（主动得分能力）

- Complete Lines（消除行数）：一次操作能消除多少行。直接关联得分，权重通常很高；
- Tetris
  Readiness（Tetris 准备度）：右侧是否留有 1×4 的直槽等待 I 块。这是高级 AI 的关键指标；
...
`simulateClearResult`
就是对消行后的棋盘计算得分、Combo、Back-to-Back、T-Spin 等， 供 `evaluateBoard`
在评分时参考，让 AI 能主动追求高价值消行。

### Evaluation

完成 `simulateClearResult` 模拟以后，AI 会开始评估棋盘。`evaluateBoard`
是 AI 决策的核心评分函数。它接收一个消行后的棋盘状态，计算出 4 个指标的值，然后按权重加权求和，返回一个分数。AI 通过比较不同放置方案的得分，选择最优解。
...
**权重：-0.7**。平均高度 10 行 ≈
-7 分，和消一行差不多。这个惩罚相对温和，持续施压让 AI 倾向于压低棋盘，但不会为此放弃消行机会。
...
**权重：-0.35**。不平整度 10 ≈
-3.5 分，相对温和但持续施加压力。引导 AI 保持表面平整，平整的棋盘更容易放置方块和实现多行消除。
...
**权重：20**。这是一个缩放因子。Tetris（4行消除，基础分800 × 20/4）=
200 分，Double（2行，基础分300 × 20/4）=
30 分。这个设计让 AI 明确地"追 Tetris"——一次 Tetris 的收益远大于两次 Double。
...
`evaluateBoard`
评估函数并不会回答："这是最好的一步吗？"，评估函数本身也不会移动方块，它只负责评分。它真正回答的是：**这个局面对未来是否更加有利？**。

#### 针对 Battle 模式的优化

tetris.js 已经是实现了 Human vs
AI 的对战模式，而针对对战模式的 AI 的决策，也针对性的加强了“**攻击性**”的鼓励：
...
- **说明**：一次操作消除的行数。表驱动设计，Tetris（4行）= 40 × (20/4) =
  200 分，Double（2行）= 6 × (20/4) =
  30 分。阶梯式奖励引导 AI 主动追求多行消除。
- **取值**：0-5 行，对应 lineRewards = [0, 2, 6, 12, 40, 80]

3. **Aggregate Height（总高度）**

- **权重**：-0.7（survival）/ -0.8（versus）
- **说明**：所有列高度之和，作为背景压力防止 AI 无限堆叠。平均高度 10 行 ≈
  -7 分，惩罚相对温和但持续施压。
...
- **权重**：动态（指数惩罚，×0.5）
- **说明**：超过 12 行后触发指数惩罚：-(maxHeight - 12)² ×
  0.5。给 AI 留足 120 格的堆叠空间，超过后迅速施压逼迫消行。
- **触发条件**：maxHeight > 12

5. **Bumpiness（不平整度）**

- **权重**：-0.35（survival）/ -0.4（versus）
- **说明**：相邻列高度差的绝对值之和。表面越平整越容易放置方块和实现多行消除。不平整度 10
  ≈ -3.5 分。
...
- **权重**：×0.03（归一化因子）
- **说明**：将游戏内消行得分映射到评分空间。得分 = 基础分 × B2B 倍率 × 等级 +
  Combo 加分 + All Clear 加分。
- **来源**：通过 simulateClearResult 计算返回

7. **T-Spin（T 旋）**

- **权重**：+8
- **说明**：T 块旋转插入 T 槽的消行方式。T-Spin 是高级技巧，额外奖励 8 分，优先级高于 T-Spin
  Mini。
...
- **权重**：+20
- **说明**：消行后棋盘完全清空时触发。重奖 20 分，是最高单项奖励之一。仅在 cleared >
  0 时有效。

11. **Combo（连击）**

- **权重**：×0.8
- **说明**：连续消行的次数。每次消行 combo
  +1，未消行则清零。Combo 越高额外加分越多，每次连击 +0.8 分。

12. **Attack Score（攻击力奖励，仅 Versus 模式）**

- **权重**：[0, 0, 10, 25, 50, 80]
- **说明**：对战模式下根据消行产生的攻击力给予加分。消 2 行 +10 分，消 4 行 +50 分，引导 AI 主动攻击对手。
- **换算**：攻击力 = garbageMap[linesCleared]，其中 garbageMap = [0, 0, 1, 2, 3,
  4]

虽然用代码在决策逻辑中没有全部覆盖常用的 AI 指标，但目前 tetris.js 的 AI 决策逻辑也已经足够健全了。而实际测试 AI 的表现，也很好的执行
**Well Depth（井深）** 和 **Tetris Readiness（Tetris 准备度）** 的局部规则。
...
随着搜索深度增加，可能出现的局面数量会快速增长。如果保留所有分支，搜索成本将越来越高。因此，AI 使用**Beam
Search**。
