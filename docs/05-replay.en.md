...
All of these approaches can implement Replay, but they all share a common problem: **the data size is very large**. At the same time, Replay tends to gradually deviate from the actual game. As the game becomes more complex, maintenance costs also increase.

---

## Replay is Not a Recording

Therefore, tetris.js chose a different approach to Replay. Replay does not record Canvas output, nor does it save every frame of the board. What it truly records is only the operations generated by the player. For example:
...
These operations are all converted into unified Commands. Replay saves exactly these Commands.
...
The final game result will definitely be the same. Therefore, Replay does not need to save:
...
These states will all be naturally recalculated. Replay only needs to re-enter the same Commands, and the entire game will run again.
...
This not only guarantees Replay but also enables AI debugging, bug reproduction, and future network synchronization.

---

## Replay and AI

Replay saves Commands, and AI outputs Commands as well. Therefore, Replay doesn't need to know whether the operation came from a player or from AI. Because Runtime always executes a unified data flow.

This means AI games can also generate Replays. When replaying, there is no need to care about who actually performed the operations at the time.
...
During development, as long as the same set of Commands can be reproduced, the entire game can return to that state at that time. This is one of the greatest advantages of Runtime.
