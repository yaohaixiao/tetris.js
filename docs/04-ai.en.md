...
Why does `simulateClearResult` need to be handled this way? Here we need to understand the **common Tetris AI decision metrics**. Tetris AI decision metrics are typically divided into three categories: **State Metrics**, **Clear Metrics**, and **Special Technique Metrics**.

### State Metrics (Real-time Board Health Assessment)

- **Holes**: The number of empty cells below occupied cells in the board. The most important metric with the highest weight.
- **Height**: The height of the tallest column, or the average column height. An overly high board compresses maneuverability and risks topping out at any moment.
- **Bumpiness**: The sum of height differences between adjacent columns. A flatter surface makes it easier to place pieces and clear lines.
- **Aggregate Height**: The sum of all column heights. Some AIs use this instead of average height.
- **Column Transitions**: The number of empty→filled or filled→empty transitions in each column, reflecting the "fragmentation" of the board.
- **Well Depth**: The depth of deep slots blocked on both sides. I-pieces require a straight well to clear 4 lines, so AI needs to consciously leave "wells."

### Clear Metrics (Active Scoring Ability)

- **Complete Lines**: How many lines can be cleared in one operation. Directly related to scoring, weight is usually very high.
- **Tetris Readiness**: Whether there is a 1×4 straight well on the side waiting for an I-piece. This is a key metric for advanced AI.

### Special Technique Metrics (T-Spin / All Clear)

- **T-Slot**: Whether there is a slot on the board where a T-piece can be inserted via rotation. Detects whether 3 of the 4 diagonal cells around a T-piece are occupied.
- **All Clear Potential**: Whether the current layout has the opportunity to completely clear the board within a few steps.
- **Combo**: The number of consecutive clears. Each clear (even just 1 line) counts as one combo.

`simulateClearResult` calculates score, Combo, Back-to-Back, T-Spin, etc., on the board after line clearing, providing reference for `evaluateBoard` during scoring, allowing AI to actively pursue high-value clears.

## Evaluation

After completing the `simulateClearResult` simulation, AI begins to evaluate the board. `evaluateBoard` is the core scoring function for AI decision-making. It receives a board state after line clearing, calculates the values of 4 metrics, then performs a weighted sum and returns a score. AI selects the optimal solution by comparing the scores of different placement options.
...
A cell is determined to be a hole when two conditions are met simultaneously: the cell itself is empty, and there is at least one block above it in the same column. Traverse each column, count from the first block from the top downward, and count empty cells encountered.
...
**Weight: -8**. One hole deducts 8 points. A normal single line clear only yields 20 points (completeLines weight × clear coefficient), so one hole is equivalent to offsetting the benefit of 2-3 cleared lines. AI will almost never leave a hole for any short-term gain.

#### Height

Takes the average height of all columns. Column height = distance from the bottom to the first empty cell.

```js
/**
 * # Get height of a specified column
 *
 * Scans from top to bottom, finds the first occupied cell,
 * returns the distance from that cell to the bottom as the column height.
 * Returns 0 if the entire column is empty.
 *
 * @example
...
**Weight: -0.7**. Average height of 10 rows ≈ -7 points, similar to clearing one line. This penalty is relatively mild, applying constant pressure to encourage AI to keep the board low, but not so severe that it abandons clear opportunities.
...
**Weight: -0.35**. Bumpiness of 10 ≈ -3.5 points, relatively mild but exerts constant pressure. Guides AI to keep the surface flat, as a flatter board makes it easier to place pieces and achieve multi-line clears.

#### Complete Lines (Clear Score)

Clear score is not calculated directly, but returned via `simulateClearResult`. It considers:
...
Clear score = Base score × B2B multiplier × level + Combo bonus + All Clear bonus.

**Weight: 20**. This is a scaling factor. Tetris (4-line clear, base score 800 × 20/4) = 200 points, Double (2-line clear, base score 300 × 20/4) = 30 points. This design gives AI a clear "pursue Tetris" goal — one Tetris is worth much more than two Doubles.
...
`evaluateBoard` does not answer: "Is this the best move?" The evaluation function itself does not move pieces; it is only responsible for scoring. What it truly answers is: **Is this board state more favorable for the future?**

#### Optimization for Battle Mode

tetris.js has already implemented Human vs AI battle mode, and for the AI decisions in battle mode, the "aggressiveness" encouragement has been specifically strengthened:
...
- **Description**: The number of empty cells blocked by blocks above. A single hole severely limits subsequent operations, making it the highest-weighted negative metric. AI avoids leaving holes at almost all costs.
- **Calculation**: Scan each column from top to bottom; after encountering a block, all empty cells below it are counted as holes.

2. **Complete Lines**
- **Weight**: 20 (survival) / 25 (versus)
- **Description**: The number of lines cleared in a single operation. Table-driven design: Tetris (4 lines) = 40 × (20/4) = 200 points, Double (2 lines) = 6 × (20/4) = 30 points. The tiered rewards guide AI to actively pursue multi-line clears.
- **Values**: 0-5 lines, corresponding to lineRewards = [0, 2, 6, 12, 40, 80]

3. **Aggregate Height**
- **Weight**: -0.7 (survival) / -0.8 (versus)
- **Description**: The sum of all column heights, serving as background pressure to prevent infinite stacking. Average height of 10 rows ≈ -7 points, providing relatively mild but constant pressure.
- **Calculation**: Each column height is counted from the bottom up to the first block, then all are summed.

4. **Max Height**
- **Weight**: Dynamic (exponential penalty, ×0.5)
- **Description**: Triggers exponential penalty after exceeding 12 rows: -(maxHeight - 12)² × 0.5. Gives AI ample stacking space (120 cells total), but rapidly increases pressure beyond that to force line clears.
- **Trigger Condition**: maxHeight > 12

5. **Bumpiness**
- **Weight**: -0.35 (survival) / -0.4 (versus)
- **Description**: The sum of absolute height differences between adjacent columns. A flatter surface makes it easier to place pieces and achieve multi-line clears. Bumpiness of 10 ≈ -3.5 points.
- **Calculation**: Σ|heights[i] - heights[i+1]|

6. **Clear Score**
- **Weight**: ×0.03 (normalization factor)
- **Description**: Maps in-game clear score to the scoring space. Score = Base score × B2B multiplier × level + Combo bonus + All Clear bonus.
- **Source**: Calculated and returned via simulateClearResult

7. **T-Spin**
- **Weight**: +8
- **Description**: A clear achieved by rotating a T-piece into a T-slot. T-Spin is an advanced technique, awarding an extra 8 points, with priority over T-Spin Mini.

8. **T-Spin Mini**
- **Weight**: +3
- **Description**: A simplified version of T-Spin, awarding 3 points. Only takes effect when standard T-Spin is not triggered; the two are mutually exclusive.

9. **Back-to-Back**
- **Weight**: +5
- **Description**: Triggered when two consecutive clears are both big moves (Tetris or T-Spin). Awards an extra 5 points, stacked on top of the clear score.

10. **All Clear**
- **Weight**: +20
- **Description**: Triggered when the board is completely empty after clearing. Awards a heavy bonus of 20 points, one of the highest individual rewards. Only effective when cleared > 0.

11. **Combo**
- **Weight**: ×0.8
- **Description**: The number of consecutive clears. Each clear increases combo by 1, reset to 0 when no clear occurs. Higher combos yield more bonus points, with each combo adding +0.8 points.

12. **Attack Score (Versus Mode Only)**
- **Weight**: [0, 0, 10, 25, 50, 80]
- **Description**: In versus mode, rewards based on attack power generated by clears. Clearing 2 lines gives +10 points, clearing 4 lines gives +50 points, guiding AI to actively attack opponents.
- **Conversion**: Attack power = garbageMap[linesCleared], where garbageMap = [0, 0, 1, 2, 3, 4]

Although not all common AI metrics are covered in the decision logic, the current AI decision logic in tetris.js is already robust enough. In actual testing, the AI also performs well in executing the local rules of **Well Depth** and **Tetris Readiness**.
