...
接收方自己的 Runtime 会触发 `FLUSH_GARBAGE`
事件给 BattleController，最终执行刷新垃圾行到棋盘：
...
Battle 模块除了 BattleController 以外，另外需要重点介绍一下就是
`GarbageSystem`。实际上它只有两个职责：
...
`calculateGarbage`
方法将玩家的消行数量转换为对对手的垃圾行攻击数量，这是对战系统中**攻击计算**的核心函数。
...
`applyGarbage`
方法在对手棋盘底部添加指定数量的垃圾行，模拟受到攻击的效果。这是对战系统中**垃圾行生成**的核心函数。
...
说到 Battle 模块，最后需要说一下的与之相关的一个话题就是：`**Scoped Event**`。

**Scoped
Event（作用域事件）**是 tetris.js 中 EventBus 的一种事件命名隔离机制。它的核心作用是：让同一个事件名称在不同 Game 实例之间互不干扰。

这里拿 `GameEvents` 的作为示例：
...
这里的 uuid 是在 Game 实例初始化的时候生成的，所有的 GameEvents 事件名称都使用了
`**uuid**` 作为 Scoped Event 的作用域。

### 为什么需要 Scoped Event？

在 Battle 模式中，两个 Game 实例（P1 和 P2）同时在运行。如果没有事件隔离，会出现以下问题：

```js
// 没有隔离：P1 和 P2 共享同一个事件
...
没有隔离：P1 和 P2 共享同一个事件，两个监听器都会被触发，造成混乱。采用 Scoped
Event 后则不会出现这个问题：

```js
// 有隔离：每个 Game 实例拥有独立的事件命名空间
...
Scoped
Event 让每个 Game 实例拥有自己的"事件频道"，就像每个玩家拥有独立的手柄一样——操作只影响自己，不会串到别人那里。
