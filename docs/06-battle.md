...
**Scoped Event（作用域事件）**是 tetris.js 中 EventBus 的一种事件命名隔离机制。它的核心作用是：让同一个事件名称在不同 Game 实例之间互不干扰。
...
这里的 uuid 是在 Game 实例初始化的时候生成的，所有的 GameEvents 事件名称都使用了 `**uuid**` 作为 Scoped Event 的作用域。
...
// 两个监听器都会被触发，造成混乱
EventBus.on('game:start', () => { /* P1 的启动逻辑 */ });
EventBus.on('game:start', () => { /* P2 的启动逻辑 */ });
```

没有隔离：P1 和 P2 共享同一个事件，两个监听器都会被触发，造成混乱。采用 Scoped Event 后则不会出现这个问题：
...
// P1 只监听 P1 的事件
EventBus.on(G1.START, () => { /* 只处理 P1 的启动 */ });
EventBus.on(G2.START, () => { /* 只处理 P2 的启动 */ });
```
Scoped Event 让每个 Game 实例拥有自己的"事件频道"，就像每个玩家拥有独立的手柄一样——操作只影响自己，不会串到别人那里。
