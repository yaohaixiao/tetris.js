 ...
 * @param {string} cmd.action - 命令的动作类型（如 MOVE_LEFT、ROTATE、DROP 等）
 * @param {object} cmd.payload - 命令携带的额外参数（通常包含 Game 实例引用）
 * @param {object} context - 执行上下文对象
 * @param {string} context.mode - 游戏当前模式（main-menu、playing、paused 等）
 * @returns {void}
 */
