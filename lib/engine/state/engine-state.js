 ...
 * # 游戏全局配置
 *
 * 集中管理游戏中所有可配置的常量，是整个游戏的"配置中心"。
 * 包括游戏模式、对战规则、棋盘尺寸、方块渲染风格、DOM 元素 ID 绑定等。
 ...
 * ## 方块渲染配置（Block）
 *
 * 控制方块的外观风格，支持 7 种渲染风格和 4 种图案纹理。
 * 渲染风格和图案可以自由组合（如 frosted + tee）。
 *
 * ### 渲染风格（Block.style）
 *
 * | 值       | 说明             | 视觉特征                           |
...
const EngineState = {
  ...
  Elements: {
    ...
    Hud: {
       ...
       * 控制者标识 DOM 元素 ID
       *
       * 显示当前由谁控制：HUMAN（人类玩家）或 AI（人工智能）。
       * 在对战模式中用于区分 P1 和 P2 的控制者。
       */
