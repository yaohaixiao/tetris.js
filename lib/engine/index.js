 ...
 *     用户在模式选择界面选择模式
 *       → CONFIRM action
 *         → 发送 engine:update:mode 事件
 *           → Engine._onUpdateMode() 更新 Store.Mode
 *         → 发送 engine:start 事件
 *           → Engine._onStart() 销毁当前实例 + 重新 launch
 *             → Engine.destroy() 清理所有子系统
 *             → Engine.launch() 根据新模式重建所有子系统
 *
 * @namespace Engine
 */
const Engine = {
  ...
  initialize: (options = {}) => {
     ...
     * ======== 步骤 1：创建引擎全局状态管理器 ========
     *
     * EngineStore 合并默认 EngineState 和传入的 options， 通过 structuredClone 深拷贝确保状态独立性。
     */
     ...
     * ======== 步骤 2：创建界面渲染器并渲染 DOM ========
     *
     * EngineRenderer 根据 Store 中的 Mode 和 Players 配置 生成对应数量和结构的 HTML 模板，一次性注入根容器。
     */
     ...
     * ======== 步骤 3：创建全局调度器 ========
     *
     * Scheduler 是所有时间驱动逻辑的核心。 挂载在 Engine 上，供所有子模块共享。
     */
     ...
     * ======== 步骤 4：标准化配置 ========
     *
     * 将 Scheduler 注入配置，并标记默认启用 AI 模式。 扩展运算符确保原始 options 不被修改。
     */
     ...
     * ======== 步骤 6：处理玩家列表 ========
     *
     * 创建 Players 数组的副本。 Single 模式移除最后一个玩家（只保留第一个）。
     */
     ...
     * ======== 步骤 8：创建对战控制器 ========
     *
     * 仅在对战模式下创建 BattleController。 注入双方 Game 实例、目标分数、UI 元素配置和玩家列表。
     */
  ...
  launch: (options = {}) => {
    ...
    for (const Game of Engine.Games) {
      ...
      /** 1. 初始化棋盘数据。Store.resetBoard() 创建空的棋盘二维数组。 */
      Store.resetBoard();

      /** 2. 加载本地存储的最高分。从 localStorage 读取键名为 tetris-high-score 的值。 */
      Game.loadHighScore();

      /** 3. 设置初始模式，初始化开始界面状态。包括预览方块队列、初始等级等。 */
      Game.setBeginningState(mode);

      /** 4. 更新 DOM 节点中 data-mode 值，同步当前模式。UI 层根据 data-mode 决定渲染内容。 */
      UI.updateMode(mode);

      /** 5. 根据窗口尺寸适配画布大小。计算 blockSize、fontSize 等渲染参数。 */
      UI.resize();

      /** 6. 初始化 HUD 信息显示。将 Store 中的分数、等级、行数等数据渲染到 DOM。 */
      UI.updateHud();

      /** 7. 更新控制者标识显示。显示 Human 或 AI 控制器类型。 */
      UI.updateController(Store.getController());

      /** 8. 延迟渲染主菜单 UI。等待字体等资源加载完成后再渲染，避免首屏白屏。 */
      UI.lazyRender();

      /** 9. 绑定输入设备的事件处理器。启动键盘、手柄、触屏和 AI 的输入事件监听。 */
  ...
  tick: (timestamp) => {
    ...
    /** 首次运行时初始化时间基准。 为所有 Game 实例设置初始累积时间。 */
     ...
     * ======== 步骤 1：驱动调度器 ========
     *
     * 执行所有到期的定时任务（delay、interval、sequence）。 这包括 AI 的决策循环、音效序列、动画时序等。
     */
    ...
    for (const Game of Games) {
      ...
      /** 检查是否有阻塞动画（如消行动画、倒计时、升级特效、垃圾行动画）。 阻塞动画期间暂停游戏逻辑，防止玩家在动画期间操作。 */
       ...
       * ======== 步骤 4：手柄状态更新 ========
       *
       * 每帧读取手柄输入状态，将新的输入转换为 command 入队。 使用可选链，手柄未连接时跳过。
       */
       ...
       * ======== 步骤 7：合并/清理动画队列 ========
       *
       * 合并待注册动画到活跃队列，移除已结束的动画。 不执行帧更新，时间逻辑全部交给 Scheduler。
       */
      Animations.flush();
       ...
       * ======== 步骤 10：叠加渲染动画特效 ========
       *
       * 在游戏界面上叠加渲染消行闪光、升级特效、垃圾行预警/闪烁等动画层。 按 layer 值从小到大渲染。
       */
      Animations.render();
    }
  ...
  unsubscribe: () => {
    ...
    // 取消 Engine 内部的 dispatch 事件 + 全局 engine 事件
    Engine._unsubscribe();
  ...
  _subscribe: () => {
    ...
    for (const Game of Games) {
       ...
       * ======== dispatch:command 事件 ========
       *
       * 处理回放系统的命令执行。 注入动画阻塞状态，统一走 dispatchCommand 管线。 根据当前 mode 路由到对应的 action
       * handler。
       */
       ...
       * ======== dispatch:input 事件 ========
       *
       * 处理键盘、手柄、AI 等实时输入。 注入动画阻塞状态和回放时间戳。 将原始输入转换为标准 game command 后入队。
       */
  ...
  _onDispatchCommand: (cmd) => {
    ...
    /** 检查当前是否有阻塞动画。 消行、倒计时、升级动画会阻塞命令执行。 */
    const isBlocked = Animations.hasBlocking([
      'clear-lines',
      'countdown',
      'level-up',
    ...
    /** 将 replay command 注入命令系统。 统一走 dispatchCommand 管线，根据 mode 路由到对应 handler。 */
  ...
  _onStart: (options = {}) => {
    ...
    /** 深拷贝当前 Store 状态。 使用 structuredClone 确保新实例的状态独立于旧实例。 */
    const cloned = structuredClone({
      ...Engine.Store.getState(),
      isRelaunch,
    });

    /** 退出模式（isRelaunch = false）时，强制切换为单人模式。 例如从对战模式退出时，回到单人模式选择界面。 */
    if (!isRelaunch) {
      cloned.Mode = 'single';
    }
