...
import EffectsRuntime from '@/lib/runtime/effects-runtime.js';
...
const Engine = {
  ...
  launch: () => {
    ...
    Engine.Keyboard = new Input.Keyboard();
  ...
  subscribe: () => {
    const { gameBoard } = UI.Canvas;

    Replay.subscribe();
    AudioRuntime.subscribe();
    GameRuntime.subscribe();
    UIRuntime.subscribe();
    EffectsRuntime.subscribe({ gameBoard });
