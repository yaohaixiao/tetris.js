 ...
 * ## 模式选择集成
 *
 * 当 Store.getMode() 返回 null 时（模式选择界面），$container.dataset.mode 被设置为 'selecting'，
 * 此时容器内不包含任何游戏界面模板，由 Scene Manager 负责渲染模式选择 Canvas 画面。
 *
 * @augments Base
...
class EngineRenderer extends Base {
  ...
  _initializeTemplates() {
    ...
    if (isVersus) {
       ...
       * 逻辑对应。
       *
       * 注意：如果 Players 为空（模式选择状态），则不执行 pop 操作。
       */
      finalPlayers.pop();
    }
  ...
  render() {
     ...
     * 设置容器 data-mode 属性。
     *
     * 用于 CSS 样式切换和 Scene Manager 的模式判断。
     *
     * - 'selecting'：模式选择界面
     * - 'single'：单人模式
     * - 'versus'：对战模式
     *
     * 当 Store.getMode() 返回 null 时，使用 'selecting' 作为默认值。
     */
