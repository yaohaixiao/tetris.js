 ...
 * ## 功能描述
 *
 * 游戏的核心协调器，负责创建和管理所有子系统，
 * 并通过事件驱动的方式协调各模块之间的交互。
 *
 * ## 核心职责
 *
 ...
 * ## 事件驱动架构
 *
 * Game 类本身不包含具体业务逻辑，而是通过 GameRouter 订阅事件，
 * 将所有操作委托给 `/lib/game/core/`、`/lib/game/logic/` 等目录中的
 ...
 *
 * 修复前 `Game.initialize()` 和 `Game.begin()` 各调用一次 `AI.start()`，
 * 导致两个 `loop()` 在 Scheduler 中交替运行，动作序列被打乱。
 *
 * ## 7-bag 独立
 *
 * 每个 Game 实例维护独立的 `this.bag = []`（在 `initialize()` 中初始化）。
 * `getBagSnapshot()` 返回 `structuredClone(this.bag)` 的深拷贝。
 * `randomShape()` 直接读写 `runtime.bag`（即 `Game.bag`），不再使用模块级全局变量。
 * Battle 模式下两个 Game 实例的 bag 完全隔离。
 *
 * ## 事件隔离
 *
 * 所有 `game:*` 事件名通过 `GameEvents(Game.id)` 生成，包含 Game 的 UUID。
 * Battle 模式下两个 Game 实例的事件完全隔离，不会互相干扰。
 *
 * @augments Base
 * @class Game
 */
