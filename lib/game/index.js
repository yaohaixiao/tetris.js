...
class Game extends Base {
  ...
  initialize() {
     ...
     * 棋盘状态、当前方块、分数、等级、行数、游戏模式、AI 统计数据等。
     *
     ...
     * 键盘/手柄输入转换为命令、命令排队和按序执行、
     * DAS（延迟自动偏移）和 ARR（自动重复速率）处理。
     */
     ...
     * P1 和 P2 键盘在开始阶段都响应，P2 在 playing 状态时禁止响应。
     */
  ...
  surrender() {
    ...
    /**
     * 发送玩家认输事件。
     * BattleRouter 收到后路由到 BattleController.surrender(loser)，
     * 将对手分数直接设为 victoryScore 并触发 over()。
     */
