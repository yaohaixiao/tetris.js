 ...
 * ## 软降计分
 *
 * 垂直向下移动（oy > 0）且非硬降时，每次移动 +1 分。 硬降另有独立的计分逻辑。
 *
 * @function move
 * @param {object} runtime - 游戏运行时对象
 * @param {number} ox - X 轴偏移量（-1 左移，1 右移，0 不动）
 * @param {number} oy - Y 轴偏移量（1 下移，0 不动）
 * @param {boolean} [isHardDrop=false] - 是否为硬降移动. Default is `false`
 * @returns {boolean} 移动成功返回 true，碰撞返回 false
 */
