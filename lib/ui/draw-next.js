...
import clearNext from './clear-next.js';
  ...
  for (let y = 0; y < shape.length; y++) {
    for (let x = 0; x < shape[y].length; x++) {
      ...
      if (shape[y][x]) {
        ...
        // 绘制填充方块与黑色边框
        nextPieceContext.fillStyle = next.color;
        nextPieceContext.fillRect(px, py, blockSize - 2, blockSize - 2);
        nextPieceContext.strokeStyle = BLACK;
        nextPieceContext.strokeRect(px, py, blockSize - 2, blockSize - 2);
      }
    }
  }
};
