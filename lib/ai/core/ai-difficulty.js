 ...
 * | `weights.completeLines` | number | 消行奖励权重（内部会平方处理）                           |
...
const AIDifficulty = {
   ...
   * | height        | -1.15 | 强力压高度，AI 极度恐高                          |
   * | bumpiness     | -0.25 | 加重不平整惩罚，要求表面平整                     |
   * | completeLines | 6     | 消 1 行奖 6 分，消 2 行奖 24 分，消 4 行奖 96 分 |
   *
   * 高度惩罚 -1.15 意味着 10 列各高 4 行时总惩罚为 -46 分。 消 4 行的奖励 96 分可以完全抵消，AI 会积极凑 Tetris
   * 来对抗恐高。
   */
  HARD: {
    lookahead: 3,
   ...
   * | height        | -1.6 | 极限压高度，与 HARD 拉开明显差距                  |
   * | bumpiness     | -0.3 | 最重不平整惩罚，表面必须平整                      |
   * | completeLines | 8    | 消 1 行奖 8 分，消 2 行奖 32 分，消 4 行奖 128 分 |
   *
   * 高度惩罚 -1.6 是四个难度中的最大值，与 HARD 的 -1.15 拉开 0.45 的差距。 消 4 行的奖励 128
   * 分是唯一能对抗高度惩罚的手段， AI 会疯狂追求一次性多行消除来保持低堆叠。
   */
  EXPERT: {
    lookahead: 4,
    noise: 0,
