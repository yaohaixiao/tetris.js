 ...
 * ## 确定性前瞻
 *
 * 从快照中的 `bag` 数组消费方块，确保同一 bag 状态下多次前瞻得到完全相同的结果。
 *
 * ## 计分状态传递
 *
 * 消行后更新 combo、backToBack、tSpin，让多步前瞻中后续步骤能正确评估 Back-to-Back 和 Combo 价值。
 *
 * ## 不可变性
 *
...
const advanceSnapshot = (snapshot, move) => {
  ...
  // 3. 计算消行计分结果（用于更新 combo/backToBack）
  const clearResult = simulateClearResult(clearedBoard, snapshot);

  // 4. 从快照的 bag 中消费下一个方块
  let bag = snapshot.bag ? [...snapshot.bag] : [];
  const nextPiece =
    bag.length > 0
      ? bag.shift()
      : snapshot.next || {
  ...
  return {
    ...
    tSpin: null, // 锁定后清空 T-Spin 标记（下一块需重新检测）
  };
};
