...
const evaluateBoard = (board, weights, clearResult) => {
  ...
  /** 默认权重配置。 调用方可通过 weights 参数覆盖任意字段。 */
  const w = {
    height: -0.6, // 背景压力：适中恐高
  ...
  /** 静态评分 = 总高度惩罚 + 危险区惩罚 + 空洞惩罚 + 不平整度惩罚 + 消行奖励 */
  const staticScore =
