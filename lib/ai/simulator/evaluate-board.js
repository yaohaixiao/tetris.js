 ...
 * | wellBonus（井奖励）       | 0.8  | 奖励留井等 I 块，深槽 × 0.8 加分，引导经典 Tetris 策略 |
 *
 * ### 计分奖励（消行价值）
 *
 * | 指标         | 权重 | 说明                                 |
 ...
 * @param {object} weights - 计算的权重信息对象，可覆盖默认的
 *   height/holes/bumpiness/completeLines
...
const evaluateBoard = (board, weights, clearResult) => {
  ...
  // 井奖励：内部列比两侧低 3 格以上时，按深度 × 0.8 加分，引导留井等 I 块
  const wellBonus = getWellBonus(heights);

  /**
   * 静态评分：棋盘结构质量
   *
   * = 总高度惩罚 + 最高列惩罚 + 空洞惩罚 + 不平整度惩罚 + 消行奖励 + 井奖励
   *
   * CompleteLines 使用平方处理，使 Tetris（4 行）的奖励远超 Single（1 行）。 wellBonus 奖励留井，与
   * completeLines 配合引导经典 Tetris 策略。
   */
  const staticScore =
