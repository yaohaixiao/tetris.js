 ...
 * - 收尾逻辑：AnimationSystem 调用 `dispose()` 时执行消行、加分、升级等操作
 *
 * ## 生命周期
 *
 * 1. `constructor` → `initialize()` → 启动闪烁序列和结束定时器，播放消行音效
 * 2. Scheduler 每 120ms 切换透明度，720ms 后标记结束
 * 3. AnimationSystem 调用 `dispose()` → 取消定时器 → 执行消行收尾逻辑
 *
 * @augments Base
...
class ClearLinesAnimation extends Base {
  ...
  initialize(options) {
    ...
    // 每 120ms 切换一次透明度，共 5 次（产生 6 个阶段：显→隐→显→隐→显→隐）
