...
class ClearLinesAnimation {
  ...
  constructor(lines) {
    ...
    Audio.Sounds.clear(lines.length - 1);
  }
   ...
   */
  stop() {
    const { store, CONSTANTS } = Game;
    const { CLEAR_LINE_SCORES, MAX_LEVEL } = CONSTANTS.GAME;
    const { ROWS, COLS } = UI.CONSTANTS.BOARD;
    ...
    // 触发升级逻辑，回放时不触发
    if (newLevel > state.level && !Replay.playing) {
      Effects.startLevelUp(newLevel);
    }
   ...
   */
  render() {
    UI.renderClear({ lines: this.lines });
  }
}
