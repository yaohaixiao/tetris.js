...
import applyClearLines from '@/lib/game/logic/apply-clear-lines.js';
...
class ClearLinesAnimation {
  ...
  stop() {
    const { store } = Game;
    const result = applyClearLines(store.getState());

    // 1. 触发升级逻辑，回放时不触发
    if (result.levelUp && !Replay.playing) {
      // 暂停 BGM
      Audio.stopBGM();
      // 播放升级提示音
      Audio.Sounds.levelUp();
      // 触发升级动画
      Effects.startLevelUp(result.level);
    }
