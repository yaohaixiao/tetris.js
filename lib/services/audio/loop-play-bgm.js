...
const loopPlayBGM = (melody, options = {}) => {
  ...
  let nextNoteTime = AudioState.audioCtx.currentTime;
  ...
  const scheduleNote = (note, time) => {
    ...
    // 只对非休止符调用 playTone，休止符仅推进时间
    if (freq > 0) {
      playTone(freq, stepDur, {
        volume,
        wave,
        gate,
        articulation,
  ...
  const scheduler = () => {
    ...
    while (
      nextNoteTime <
      AudioState.audioCtx.currentTime + SCHEDULE_AHEAD_TIME
    ) {
      // 取出当前音符
    ...
    AudioState.bgmTimer = setTimeout(scheduler, LOOKAHEAD);
  };
