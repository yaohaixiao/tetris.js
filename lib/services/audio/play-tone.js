 ...
 * @property {number} [attackTime=0.003] - 起音时间（秒），音量从 0 冲到最大值的耗时. Default is
 *   `0.003`
 * @property {number} [releaseTime=0.02] - 释音时间（秒），指数衰减开始到完全静音的耗时. Default is
 *   `0.02`
 * @property {number} [sustainRatio=0.9] - 延音比（0-1），衰减段开始时的音量占最大音量的比例. Default
 *   is `0.9`
 */
 ...
 * @property {number} [volume=0.15] - 音量峰值（0-1）. Default is `0.15`
 * @property {OscillatorType} [wave='square'] - 波形类型. Default is `'square'`
 * @property {number} [gate=1] - 发音占比（0-1），1 为连奏，小于 1 产生断奏间隙. Default is `1`
 * @property {Articulation} [articulation={}] - 运音包络参数. Default is `{}`
 * @property {number} [startTime] - 振荡器启动时间（秒），默认为当前 audioCtx.currentTime
 */
 ...
 * @param {PlayToneOptions} [options] - 播放参数
 * @returns {void}
 */
