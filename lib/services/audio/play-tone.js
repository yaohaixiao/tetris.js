...
const playTone = (audio, freq, dur, options = {}) => {
   ...
   */
  const MIN_GAIN = 0.000_01; // 1e-5，约 -100dB

  /** 安全音量值：确保 volume 是有效的正数 使用 isNumber 工具函数 + Number.isFinite 双重校验 */
  const safeVolume =
    isNumber(volume) && Number.isFinite(volume) && volume > 0 ? volume : 0.15;
  ...
  const safeSustainRatio =
    isNumber(sustainRatio) && Number.isFinite(sustainRatio) && sustainRatio > 0
      ? sustainRatio
      : 0.9;
