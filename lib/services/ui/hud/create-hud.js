import HudDom from '@/lib/services/ui/hud/hud-dom.js';
  ...
  return {
    update: (state) => {
      ...
      if (state.lines !== prev.lines) {
        setText(HudDom.lines, state.lines, 2);
        prev.lines = state.lines;
      }

      if (state.level !== prev.level) {
        setText(HudDom.level, state.level, 2);
    ...
    tick: () => {
      // 每帧分别驱动两个动画
      animationScore(scoreTracker, HudDom.score, 5);
      animationScore(highScoreTracker, HudDom.highScore, 5);
    },
