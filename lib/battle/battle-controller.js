...
import { GameEvents, AudioEvents } from '@/lib/events/event-catalog.js';
...
class BattleController extends Base {
  ...
  processAttack(from, lines) {
    ...
    if (remaining > 0) {
       ...
       * 延时序列：
       *
       * 1. 立即触发预警动画（START_GARBAGE_WARNING 事件）
       * 2. 120ms 后播放预警音效
       */
      Scheduler.delay(() => {
        // 播放预警音效（降调警示音）
  ...
  flushGarbage(game) {
    ...
    /** 提取垃圾行数据（棋盘底部 amount 行）： 用于垃圾行闪烁动画，只闪烁非空洞的垃圾方块。 */
    const garbageRows = next.slice(-amount);

    const events = GameEvents(game.id);
    const roundId = this.getRoundId();
    // 通知 Game 实例触发垃圾行闪烁动画（blocking=true，动画期间暂停游戏逻辑）
     ...
     * 延时序列：
     *
     * 1. 立即触发垃圾行闪烁动画（START_GARBAGE_PUSH 事件），传入垃圾行数据
     * 2. 120ms 后播放垃圾行插入音效
     */
    Scheduler.delay(() => {
      // 播放垃圾行插入音效（低沉下行音效）
