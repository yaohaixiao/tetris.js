 ...
 * | 职责             | 说明                                                           |
 * | ---------------- | -------------------------------------------------------------- |
 * | **子系统协调**   | 创建并管理 VersusState、BattleHUD、BattleUI、BattleRouter 实例 |
...
class BattleController extends Base {
   ...
   */
  initialize() {
    // 从配置中解构 Game 实例数组和 DOM 元素配置
    const { games, elements } = this;

    /**
     * ======== 步骤 1：创建状态管理器 ========
     *
     * VersusState 负责管理对战的所有状态数据：
     *
     * - Running：对战是否进行中
     * - Winner：当前单局胜者
     * - Scores：双方胜场数
     ...
     * ======== 步骤 2：创建 HUD 控制器 ========
     *
     * BattleHUD 负责实时更新 DOM 中的分数显示。 需要 games（获取 Player 信息）和 state（查询分数）。
     */
    this.state = state;
     ...
     *
     * 注意：此时 this 已完整初始化（state、hud 已就绪）， 可以安全地将自身引用传递给路由器。
     */
     ...
     * ======== 步骤 4：创建结果展示界面 ========
     *
     * BattleUI 负责整场对战结束后的结果展示。 需要 elements 配置来定位 DOM 元素。
     */
