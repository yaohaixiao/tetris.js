 ...
 * @param {object} animation - 动画对象，必须包含 update、render 方法和 layer、blocking、name
 *   属性
 * @returns {void}
 */
export const registerAnimation = (animation) => {
 ...
 * # 渲染所有动画
 *
 * 按照 layer 属性排序，确保动画按正确的层级顺序渲染
 *
 * Layer 值越大的动画越靠近屏幕上层（后渲染会覆盖先渲染的）
 *
...
export const hasBlockingAnimation = (names = []) =>
  animations.some((animation) => {
    ...
    return names && names.length > 0
      ? isBlocking && names.includes(animation.name) // 特定动画且阻塞
      : isBlocking; // 任何阻塞动画
  });
