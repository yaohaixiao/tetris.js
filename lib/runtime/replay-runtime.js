...
const Replay = {
  ...
  subscribe() {
    EventBus.on('replay:start:record', () => {
      Replay.startRecord();
    });

    EventBus.on('replay:stop:record', () => {
      Replay.stopRecord();
    });

    EventBus.on('replay:add:record', (record) => {
      if (!Replay.recording) {
        return;
      }
    ...
    EventBus.on('replay:add:piece', (piece) => {
      if (!Replay.recording) {
        return;
      }
    ...
    EventBus.on('replay:start:play', () => {
      Replay.startPlay();
    });

    EventBus.on('replay:reset', () => {
      Replay.reset();
    });

    EventBus.on('replay:game:over', () => {
      if (Replay.hasData) {
    ...
    EventBus.on('replay:stop:clear:lines', ({ isLevelUp, level }) => {
  ...
  syncPlayElapsed({ timestamp, isBlocked }) {
    ...
    if (elapsedDelta > 1000) {
      // 把 startTime 往后拨，让 playElapsed 只增加 1 秒
      Replay.startTime += elapsedDelta - 1000;
      Replay.playElapsed = prevElapsed + 1000;
    } else {
   ...
   */
  update(context) {
    const { getSpeed, timestamp } = context;
    ...
    if (nextCmd) {
      const dropInterval = getSpeed?.() ?? 1000;
    ...
    while (
      ...
      EventBus.emit('dispatch:command', cmd);
