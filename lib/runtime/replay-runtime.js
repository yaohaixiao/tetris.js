...
const Replay = {
  ...
  update(context) {
    ...
    if (nextCmd) {
      const dropInterval = Game.getSpeed?.() ?? 1000; // 根据你的实际获取方式调整
      if (nextCmd.ms > elapsedTime + dropInterval * 2) {
        // 把 playElapsed 调到刚好能触发下一个指令
        Replay.playElapsed = nextCmd.ms;
        /*
         * 注意：这里改的是 playElapsed，不是 startTime
         *
         * 但 playElapsed 的计算依赖 startTime，所以需要同步调整 startTime
         */
