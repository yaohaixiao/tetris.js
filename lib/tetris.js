...
/**
 * #返回主菜单（等级选择界面） 停止游戏、清空状态、重置数据，并切换到等级选择界面
 *
 * @function backToMenu
 * @returns {void}
 */
const backToMenu = () => {
  ...
  gameState.isSelectLevel = true; // 进入等级选择界面
  gameState.score = 0;
  gameState.lines = 0;
  gameState.level = 1;
  gameState.curr = null;
 ...
 * @returns {void}
 */
const handleLevelSelect = (key, lowerKey) => {
  // 数字键 1-9 设置对应等级
  if (key >= '1' && key <= '9') {
 ...
 */
const handleGlobalKeys = (lowerKey) => {
  // 全局快捷操作映射
  const actions = {
 ...
 * 用于退出隐藏模式的触发流程
 *
 * @function handleStopHold
 * @param {KeyboardEvent} e - 键盘事件对象
 */
const handleStopHold = (e) => {
  // 判断松开的是否是 P 键
 ...
 *
 * @function handleMainActions
 * @param {KeyboardEvent} e - 键盘事件对象
 * @returns {boolean} 是否阻止后续操作
 */
const handleMainActions = (e) => {
  // 获取按下的键名与小写键名
  const { key } = e;
...
const bindEvents = () => {
  ...
  // 监听键盘按下事件，处理所有游戏操作
  document.addEventListener('keydown', handleMainActions);
