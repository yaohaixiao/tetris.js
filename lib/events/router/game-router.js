...
class GameRouter extends Base {
   ...
   */
  subscribe() {
    // 获取所有子模块和 Game 实例的引用（由 Base 构造函数注入）
  ...
  _onUpdateModeIndex = (payload) => {
    ...
    // 发送 UI 更新事件（Scene Manager 监听后重绘光标）
   ...
   * ## 更新手柄连接状态
   *
   * 更新 Store 中的手柄连接状态，并在游戏进行中时触发通知动画。
   *
   * @private
   * @param {object} options - 参数对象
  ...
  _onSelectLevel = (options) => {
    ...
    const { level } = options;
    const events = UIEvents(Game.id);
    // 调用 Game 的 selectLevel 方法（含音效播放）
