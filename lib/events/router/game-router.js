 ...
 * | UPDATE_MODE_INDEX        | `_onUpdateModeIndex`        | 更新模式选择光标        |
 * | UPDATE_BATTLE_INDEX      | `_onUpdateBattleIndex`      | 更新对战选择光标        |
 * | UPDATE_MODE              | `_onUpdateMode`             | 更新游戏模式 + UI 通知  |
 * | UPDATE_LEVEL             | `_onUpdateLevel`            | 更新等级                |
 * | UPDATE_GAMEPAD_CONNECTED | `_onUpdateGamepadConnected` | 更新手柄连接状态 + 通知 |
 * | SWITCH_CONTROLLER        | `_onSwitchController`       | human ↔ ai 切换         |
 * | UPDATE_HUD               | `_onUpdateHud`              | 刷新 HUD 显示           |
 * | SAVE_HIGH_SCORE          | `_onSaveHighScore`          | 保存最高分              |
 * | SWITCH_TO_GAME_MODE      | `_onSwitchToGameMode`       | 切换到游戏模式选择      |
 * | SWITCH_TO_BATTLE_MODE    | `_onSwitchToBattleMode`     | 切换到对战模式选择      |
 ...
 * | GET_GHOST_POSITION       | `_onGetGhostPosition`       | 获取 ghost 定位         |
 ...
 * | START_GARBAGE_WARNING    | `_onStartGarbageWarning`    | 垃圾行预警动画          |
 * | START_GARBAGE_PUSH       | `_onStartGarbagePush`       | 垃圾行闪烁动画          |
 * | TOGGLE_BGM               | `_onToggleBGM`              | 背景音乐切换            |
 * | REPLAY_PREPARE           | `_onReplayPrepare`          | 回放准备                |
 * | SURRENDER                | `_onSurrender`              | 对战认输                |
 *
 * @augments Base
 * @class GameRouter
 */
class GameRouter extends Base {
   ...
   */
  subscribe() {
    // 获取所有子模块和 Game 实例的引用（由 Base 构造函数通过 inject 注入）
  ...
  _onUpdateModeIndex = (payload) => {
    ...
    // 发送 UI 更新事件，Scene Manager 监听后重绘光标位置
  ...
  _onSelectLevel = (options) => {
    ...
    const { level } = options;
    const events = UIEvents(Game.id);
    // 调用 Game 的 selectLevel 方法（含音效播放和状态更新）
  ...
  _onSwitchToDifficulty = () => {
    const { Game } = this;
    const events = UIEvents(Game.id);
    // 通知 UI 更新模式为 difficulty
    this.emit(events.UPDATE_MODE, { mode: 'difficulty' });
    // 调用 Game 切换难度选择界面（含音效播放）
  ...
  _onGameStart = () => {
    ...
    // 调用 Game 启动游戏流程（含倒计时动画）
    Game.start();

    // 如果当前是 AI 控制，发送 AI 启动事件
   ...
   * ### roundId 和 Battle 参数
   *
   * 动画初始化时记录当前 roundId 和 Battle 引用。 每帧渲染时检查 roundId 是否匹配，不匹配则自动标记为 `_finished`，
   * 防止上一局残留的动画在新一局中继续渲染。
   *
   * @private
   * @param {object} payload - 参数对象
   ...
   * ### roundId 和 Battle 参数
   *
   * 同预警动画，用于动画过期检测。 新一局开始时 roundId 递增，旧动画渲染时发现不匹配自动标记结束。
   *
   * @private
   * @param {object} payload - 参数对象
