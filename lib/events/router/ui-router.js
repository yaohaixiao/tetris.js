 ...
 * | HUD 绘制 | UPDATE_CONTROLLER  | \_onUpdateController | 更新控制器类型显示（玩家/AI）  |
 * | 画布绘制 | RESIZE             | \_onResize           | 响应窗口大小变化，调整画布尺寸 |
 * | 动画特效 | RENDER_NEXT_PIECE  | \_onRenderNextPiece  | 渲染预览方块                   |
...
class UIRouter extends Base {
  ...
  subscribe() {
    const { Game } = this;
    // 获取 UI 事件名称（带游戏 ID 命名空间）
    const events = UIEvents(Game.id);

    /* ---------- HUD 绘制 ---------- */
    // 更新游戏模式显示（playing、replay、game-over 等）
