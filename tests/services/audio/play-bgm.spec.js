...
jest.mock('@/lib/services/audio/constants/musics', () => ({
  ...
  Ascension:        { melody: 'melody-9',  duration: 200, volume: 0.08, wave: 'triangle', gate: 1,   articulation: {} },
  NeonNights:       { melody: 'melody-10', duration: 160, volume: 0.08, wave: 'triangle', gate: 0.85, articulation: {} },
  FrozenPeaks:      { melody: 'melody-11', duration: 200, volume: 0.07, wave: 'sine',     gate: 1,   articulation: {} },
  CyberRush:        { melody: 'melody-12', duration: 120, volume: 0.09, wave: 'sawtooth', gate: 0.6, articulation: {} },
  Starlight:        { melody: 'melody-13', duration: 180, volume: 0.06, wave: 'sine',     gate: 1,   articulation: {} },
  FinalPush:        { melody: 'melody-14', duration: 140, volume: 0.1,  wave: 'square',   gate: 0.8, articulation: {} },
  JourneyToWest:    { melody: 'melody-15', duration: 110, volume: 0.06, wave: 'square',   gate: 1,   articulation: {} },
}));
...
describe('playBGM', () => {
  ...
  describe('getMusicByLevel', () => {
     ...
     *
     * 等级区间：
     *   1-16   → index 0  (TetrisTheme)
     *   17-32  → index 1  (SpringFestival)
     *   33-48  → index 2  (FirstDivision)
     *   ...
     *   241-256 → index 15 (JourneyToWest)
     */

    it('level 1 → TetrisTheme (index 0)', () => {
      playBGM(audio, 1);
      expect(loopPlayBGM).toHaveBeenCalledWith(audio, 'melody-0', expect.any(Object));
    });
    ...
    it('level 17 → SpringFestival (跨入下一区间)', () => {
      playBGM(audio, 17);
      expect(loopPlayBGM).toHaveBeenCalledWith(audio, 'melody-1', expect.any(Object));
    });

    it('level 33 → FirstDivision', () => {
      playBGM(audio, 33);
      expect(loopPlayBGM).toHaveBeenCalledWith(audio, 'melody-2', expect.any(Object));
    });

    it('level 241 → JourneyToWest (最后一首区间)', () => {
      playBGM(audio, 241);
      expect(loopPlayBGM).toHaveBeenCalledWith(audio, 'melody-15', expect.any(Object));
    });
