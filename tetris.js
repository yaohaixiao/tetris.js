var tetris = (() => {
  ...
  var Replay = {
    ...
    subscribe() {
      ...
      event_bus_default.on("replay:stop:clear:lines", ({ isLevelUp, level: level2 }) => {
  ...
  var getNextPiece = () => {
    if (replay_runtime_default.playing) {
      ...
      const next3 = replay_runtime_default.pieceSequence[replay_runtime_default.pieceIndex] || null;
  ...
  var getSpeed = () => {
    ...
    return Math.max(180, 1e3 - (level2 - 1) * Math.floor(1e3 / (configuration_default.Level.max * 0.7)));
  };
  ...
  var applyClearLines = () => {
    ...
    return {
      stateHandler: (prev) => ({
        ...
        level: Math.min(Math.max(prev.level, newLevel), Level.max),
        board: board2
      }),
      levelUp,
      level: newLevel
