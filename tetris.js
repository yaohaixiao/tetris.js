var tetris = (() => {
  ...
  var CommandQueue = class extends core_default {
     ...
     *
     * @param {Command} command - 要入队的命令实例
     * @returns {void}
     */
    enqueue(command) {
     ...
     * @param {Command} params.cmd - 要入队的命令实例
     * @returns {void}
     */
    _onEnqueue = ({ cmd }) => {
      this.enqueue(cmd);
  ...
  // lib/ai/clone-board.js
  var cloneBoard = (board) => structuredClone(board);
  ...
  // lib/ai/generate-moves.js
  var generateMoves = ({ board, piece }) => {
    const moves = [];
    let { shape } = piece;
  ...
  var AIController = class extends core_default {
    ...
    loop = () => {
      ...
      this.aiSchedulerId = this.Scheduler.delay(
        this.loop,
        this.Game.getSpeed(),
      );
    };
     ...
     */
    think(state) {
      const board = state.board.map((row) => row.map((cell) => (cell ? 1 : 0)));
      const piece = {
      ...
      const moves = generate_moves_default({
        board,
        piece,
      });
      let best = null;
      ...
      for (const move2 of moves) {
        const score = evaluate_board_default(move2.board);
        if (score > bestScore) {
          bestScore = score;
          best = move2;
        }
  ...
  var FIREWORK_COLORS = [TEAL4, YELLOW4, PURPLE4, ORANGE4, GREEN4, RED4];
  ...
  var spawn = (game) => {
    ...
    const state = Store.getState();
    if (collision_default2(game, 0, 0)) {
      over_default(game);
      return;
    }
  ...
  var move = (game, ox, oy) => {
    ...
    const state = Store.getState();
    let { cx, cy } = state;
    if (!collision_default2(game, ox, oy)) {
      cx += ox;
      cy += oy;
      Store.setState({
        cx,
  ...
  var Game = class extends core_default {
    ...
    /** @returns {void} */
    startPaused() {
      this.effect = new paused_animation_default({ Scheduler: this.Scheduler });
    ...
    startClearLines(linesToClear) {
      this.Animations.register(
        new clear_lines_animation_default({ Game: this, lines: linesToClear }),
      );
    }
    /**
     * ## 开始升级特效
  ...
  var DIFFICULT_ACTIONS = {
    ...
    NORMAL: (payload) => {
      ...
      Game2.emit(`game:${Game2.id}:select:difficulty`, {
        difficulty: 'normal',
      });
    },
    /**
  ...
  var GAME_PLAYING_ACTIONS = {
    ...
    SWITCH_CONTROLLER: (payload) => {
      ...
      Game2.emit(`game:${Game2.id}:swtich:controller`);
    },
  };
