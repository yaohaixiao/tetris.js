var tetris = (() => {
  ...
  var applyGravity = (board, clearLines2) => {
    ...
    for (let x = 0; x < COLS2; x++) {
      const columnBlocks = [];
      for (let y = 0; y < ROWS2; y++) {
        if (!clearLines2.includes(y) && board[y][x] !== 0) {
  ...
  var GravityFallAnimation = class {
    ...
    capture() {
      const { board, clearLines: clearLines2 } = this.state;
      const ROWS2 = board.length;
      const COLS2 = board[0].length;
      const beforeBoard = board.map((row) => [...row]);
      const afterBoard = board.map((row) => [...row]);
      apply_gravity_default(afterBoard, clearLines2);
      for (let x = 0; x < COLS2; x++) {
        const before = [];
        const after = [];
        for (let y = ROWS2 - 1; y >= 0; y--) {
          if (clearLines2.includes(y)) continue;
    ...
    stop() {
      ...
      this.render();
      const newFullLines = find_full_lines_default(this.state.board);
      if (newFullLines.length > 0) {
        this.state.clearLines = newFullLines;
        clear_lines_controller_default(newFullLines, this.state);
      }
    }
  };
  ...
  var ClearLinesAnimation = class {
    ...
    stop() {
      ...
      gravity_fall_controller_default(this.state);
    }
    /**
     * ## 渲染动画效果
     *
  ...
  var renderAnalogClock = () => {
    ...
    const { TEAL: TEAL4, RGBA_TEAL: RGBA_TEAL3, ORANGE: ORANGE4 } = colors_default;
    const { gameBoard: gameBoard2, gameBoardContext: ctx } = canvas_default;
    const { width, height } = gameBoard2;
