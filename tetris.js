var tetris = (() => {
  ...
  var Sounds = class extends core_default {
    ...
    CLEAR = (lines = 1, isPerfectClear = false) => {
      const frequencies = [
        [440, 554, 659],
        // 0: A4 + C#5 + E5 — 大三和弦（明亮，1行消除）
        [587, 740, 880],
        // 1: D5 + F#5 + A5 — 大二和弦（温暖，2行消除）
        [523, 622, 784],
        // 2: C5 + Eb5 + G5 — 小三和弦（蓝调感，3行消除）
        [659, 784, 988],
        // 3: E5 + G5 + B5 — 挂留感（紧张期待）
        [440, 659, 880],
        // 4: A4 + E5 + A5 — 纯五度开放和弦（空灵辉煌）
      ];
  ...
  var Musics = {
     ...
     * @type {Music}
     */
    Ascension: ascension_default,
    /**
     * ## 背景音乐：NeonNights
     *
     * @type {Music}
     */
    NeonNights: neon_nights_default,
    /**
     * ## 背景音乐：FrozenPeaks
     *
     * @type {Music}
     */
    FrozenPeaks: frozen_peaks_default,
    /**
     * ## 背景音乐：CyberRush
     *
     * @type {Music}
     */
    CyberRush: cyber_rush_default,
    /**
     * ## 背景音乐：Starlight
     *
     * @type {Music}
     */
    Starlight: starlight_default,
    /**
     * ## 背景音乐：FinalPush
     *
     * @type {Music}
     */
    FinalPush: final_push_default,
    /**
     * ## 背景音乐：JourneyToWest
     *
     * @type {Music}
  ...
  var MUSIC_LIST = [
    musics_default.TetrisTheme,
    musics_default.SpringFestival,
    musics_default.FirstDivision,
    musics_default.GongXiFaCai,
    musics_default.Loginska,
    musics_default.BeyondTheWall,
    musics_default.Technotris,
    musics_default.GoldenSnakeDance,
    musics_default.Korobeiniki,
    musics_default.Ascension,
    musics_default.NeonNights,
    musics_default.FrozenPeaks,
    musics_default.CyberRush,
    musics_default.Starlight,
    musics_default.FinalPush,
    musics_default.JourneyToWest,
  ];
  ...
  var applyClearLines = (runtime) => {
    const { MAX_LEVEL: MAX_LEVEL2, CLEAR_LINE_SCORES: CLEAR_LINE_SCORES2 } =
      game_default;
  ...
  var LandingFlashAnimation = class extends core_default {
    ...
    render() {
      const { state, Game: Game2 } = this;
      const events = UIEvents(Game2.id);
      this.emit(events.RENDER_LANDING_FLASH, { state: { cells: state.cells } });
    }
  };
  ...
  var Game = class extends core_default {
     ...
     * ## 构造函数
     *
     * 接收依赖配置，调用 `initialize()` 创建所有子系统。 构造函数本身不进行复杂初始化，所有子系统的创建都在
     * `initialize()` 中完成。
     *
     * @param {object} options - 配置（依赖的执行上下文）对象
