var tetris = (() => {
  ...
  var nt = (e) => {
      ...
      let t = (e - a.timestamp) / 1e3,
        o = ot(a.state);
      if (((a.timestamp = e), O.playing)) {
        let { data: r } = O;
        for (; O.cursor < r.length && r[O.cursor].frame === O.frame; ) {
          let n = r[O.cursor];
          (Y.enqueue(new ie(n.cmd.type, n.cmd.payload)), O.cursor++);
        }
      }
      (Y.flush(a),
        a.update(t),
        O.frame++,
        (!a.accumulator || e - a.accumulator > o) &&
          (rt(a.state), (a.accumulator = e)),
        a.render(),
        a.animate(),
